...
SKIP="
tests/harness/guard-check.sh|이 게이트에 넣기엔 느리다 — 다른 검사 전부를 합친 것보다 오래 걸린다(재려면 직접 돌려라). 게이트 하나가 그만큼 붙으면 구현 사이클이 그 값을 태스크마다 치른다. plugins/harness/hooks/guard.sh 를 고치는 커밋에서 손으로 돌린다.
