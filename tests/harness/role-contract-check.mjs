...
try {
  ...
  for (const runtime of ['claude', 'codex']) for (const role of roleNames) {
    ...
    const guard = (tool_name, tool_input) => spawnSync(process.execPath, [path.join(root, 'hooks/guard.mjs')], {cwd: wt, env, encoding: 'utf8', input: JSON.stringify({...event, tool_name, tool_input})}).status;
    ...
    check(guard('Bash', {command: `HARNESS_ROOT='${repo.replaceAll("\\", "/").replaceAll("'", "'\\''")}' ledger.sh note fixture#1 receipt`}) === (role === 'implementer' ? 0 : 2), `${role} ledger note permission`);
  }
