...
function codexConfig(destination) {
  const hooks = JSON.parse(fs.readFileSync(path.join(source, 'hooks/codex.json'), 'utf8').replaceAll('${CLAUDE_PLUGIN_ROOT}', destination)).hooks;
...
// Define the fixture's root hook input before it becomes a repository. Codex
// linked worktrees read declarations from the root .codex (hooks/list docs).
fs.mkdirSync(path.join(main, '.codex'));
fs.writeFileSync(path.join(main, '.codex/config.toml'), codexConfig(path.join(root, 'bundles/codex-cli')));
...
for (const surface of ['claude-cli', 'codex-cli', 'antigravity-cli']) {
  ...
  if (surface === 'codex-cli') {
    fs.writeFileSync(path.join(workspace, '.codex/config.toml'), codexConfig(destination));
