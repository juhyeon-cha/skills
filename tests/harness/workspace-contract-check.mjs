...
`);
  ...
  for (const backend of ['github', 'beads', 'notion']) {
    ...
    fs.writeFileSync(path.join(bin, 'gh'), '#!/bin/sh\ncase "$1 $2" in "pr list") printf "4242\\t%s\\n" "$(git rev-parse "$4")"; exit 0 ;; esac\nexit 97\n', {mode: 0o755});
    ...
    // A worktree branch name repeats when a story is reopened, so a merged PR whose head is not
    // this exact tip must not excuse commits it never saw -- cleanup deletes the branch.
    fs.writeFileSync(path.join(bin, 'gh'), '#!/bin/sh\ncase "$1 $2" in "pr list") printf "4242\\t%s\\n" "0000000000000000000000000000000000000000"; exit 0 ;; esac\nexit 97\n', {mode: 0o755});
    result = run('bash', [path.join(plugin, targets[1]), 'story-1'], repo, fixtureEnv);
    check(result.status === 1 && fs.existsSync(wt), 'a merged pull request on another commit does not excuse unpushed work');
