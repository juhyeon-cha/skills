...
try {
  ...
  for (const declaration of ['"name" = "harness-reviewer"', "'name' = 'harness-reviewer'", '"na\\u006de" = "harness\\u002dreviewer"', 'name = """harness-reviewer"""']) {
  ...
  fs.unlinkSync(extra);
  fs.writeFileSync(extra, 'name = "other"\n[tools]\nvalue = 1\n');
  check(verifyRegistration(registration) === registration, 'unrelated table belongs to runtime validation');
  fs.unlinkSync(extra);
