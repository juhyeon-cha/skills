...
export function loadRole(role, root = plugin) {
  ...
  if (!signals.length || new Set(signals).size !== signals.length) throw new Error('role SIGNAL contract missing/duplicate');
  return {role, source, sha256: hash(bytes), description: required(field('description'), 'description'), body: match[2], signals};
}
...
export function registerRoles(runtime, destination, root = plugin) {
  ...
  if (runtime === 'codex') {
    if (!path.isAbsolute(destination ?? '')) throw new Error('absolute agents destination required');
    ...
    for (const role of roleNames) {
      ...
      if (fs.existsSync(file) && fs.readFileSync(file, 'utf8') !== entry.text) throw new Error(`registration already differs: ${file}`);
    }
  }
  const roles = roleNames.map(role => {
    const entry = projection(runtime, role, root);
    const file = runtime === 'claude' ? entry.source : path.join(destination, `${entry.identifier}.toml`);
    if (runtime === 'codex' && !fs.existsSync(file)) fs.writeFileSync(file, entry.text, {flag: 'wx'});
    return {role, identifier: entry.identifier, source: entry.source, sourceHash: entry.sha256, file, fileHash: hash(entry.text)};
  });
  return {version: 1, runtime, root, roles};
}

export function verifyRegistration(registration) {
  if (registration?.version !== 1 || !path.isAbsolute(registration.root ?? '') || !Array.isArray(registration.roles) || registration.roles.length !== roleNames.length) throw new Error('registration missing');
  const seen = new Set();
  for (const entry of registration.roles) {
    ...
    seen.add(entry.role);
    if (entry.identifier !== expected.identifier || entry.source !== expected.source || entry.sourceHash !== expected.sha256 || entry.fileHash !== hash(expected.text) || fs.readFileSync(entry.file, 'utf8') !== expected.text) throw new Error('role registration drift');
    if (registration.runtime === 'claude' && entry.file !== expected.source) throw new Error('Claude must reference original role');
...
export function roleCall(registration, request) {
  ...
  for (const field of ['sessionId', 'parentAgentId', 'task', 'message']) required(request[field], field);
  if (!Array.isArray(request.implementerIds) || !Array.isArray(request.previousAgentIds) || [...request.implementerIds, ...request.previousAgentIds].some(id => typeof id !== 'string' || !id)) throw new Error('identity history required');
  if (request.role !== 'implementer' && !request.implementerIds.length) throw new Error('implementation identity missing');
  // History is supplied from persisted delegation/RETRY records, never guessed from prose.
  return {...request, runtime: registration.runtime, identifier: roleIdentifier(registration.runtime, definition.role), sourceHash: definition.sha256};
}

export function roleResult(registration, call, outcome) {
  try {
    const expected = roleCall(registration, call);
    if (call.identifier !== expected.identifier || call.sourceHash !== expected.sourceHash || call.runtime !== expected.runtime) throw new Error('call drift');
    const id = required(outcome?.agentId, 'child identity');
    if (outcome.state !== 'completed' || id === call.parentAgentId || (call.role !== 'implementer' && call.implementerIds.includes(id)) || call.previousAgentIds.includes(id)) throw new Error('interrupted, self judgment, or reused instance');
    ...
    if (!loadRole(call.role, registration.root).signals.includes(signal)) throw new Error('SIGNAL missing/unregistered');
    return {status: 'REACHED', signal, agentId: id, sessionId: call.sessionId, task: call.task, result};
  } catch (error) {
