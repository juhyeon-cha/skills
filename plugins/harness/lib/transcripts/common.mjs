...
export function parseRecords(bytes) {
  return bytes.toString('utf8').split(/\r?\n/).filter(line => line.trim()).map((line, index) => {
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`unsupported record ${index + 1}`);
    return value;
  });
}
...
export const usageKeys = ['input_tokens', 'cache_creation_input_tokens', 'cache_read_input_tokens', 'output_tokens'];
export function tokens(messages) {
  const known = Object.fromEntries(usageKeys.map(key => [key, 0]));
  let complete = messages.length > 0; const seen = new Map();
  ...
  return {status: complete ? 'KNOWN' : 'UNKNOWN', total: complete ? known : null, observed_partial: known, requests: seen.size};
}
