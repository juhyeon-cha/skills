...
export function decodeCodex(rows, sessionId) {
  ...
  for (const [index, row] of rows.entries()) {
    if (
      ...
      errors.push('unsupported Codex exec record');
      continue;
    }
    if (row.type === 'thread.started') {
      thread++;
      if (row.thread_id !== sessionId) errors.push('Codex thread mismatch');
    }
    ...
    if (row.type.startsWith('item.')) {
      ...
      if (
        ...
        errors.push('Codex item missing/unsupported');
        continue;
      }
...
export function codexOutcome(decoded, call, boundary = 0) {
  ...
  const spawn = decoded.tools.find(
    (item) =>
      item.id === call.nativeCallId && item.tool === 'spawn_agent' && item.index >= boundary,
  );
  if (
    !spawn ||
    spawn.event !== 'item.completed' ||
    spawn.status !== 'completed' ||
    spawn.sender_thread_id !== call.parentAgentId ||
    spawn.receiver_thread_ids?.length !== 1
  )
    throw new Error('native spawn identity/completion missing');
  const agentId = spawn.receiver_thread_ids[0];
  const later = decoded.tools.filter(
    (item) => item.index > spawn.index && item.receiver_thread_ids?.includes(agentId),
  );
  if (later.some((item) => item.tool !== 'wait'))
    throw new Error('child has follow-up/reuse or unsupported operation');
  const wait = later.at(-1);
  const state = wait?.agents_states?.[agentId];
  if (
    !wait ||
    wait.event !== 'item.completed' ||
    wait.status !== 'completed' ||
    wait.sender_thread_id !== call.parentAgentId ||
    state?.status !== 'completed' ||
    typeof state.message !== 'string'
  )
    throw new Error('native wait/instance completion missing');
