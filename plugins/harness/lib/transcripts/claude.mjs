  ...
  for (const record of parent) {
    ...
    for (const block of Array.isArray(record.message?.content) ? record.message.content : []) {
      if (
        ...
        if (!block.id || calls.has(block.id)) {
          errors.push('ambiguous Claude invocation');
          continue;
        }
        calls.set(block.id, {
          id: block.id,
          role: roleName(block.input?.subagent_type),
          reason:
            block.input?.subagent_type && !roleName(block.input.subagent_type)
              ? 'unknown invocation role'
              : undefined,
    ...
    if (result?.agentId) {
      const toolId = record.message?.content?.find?.(
        (block) => block.type === 'tool_result',
      )?.tool_use_id;
      const call = calls.get(toolId);
      if (!call) {
        errors.push(`completion/launch without invocation inventory: ${result.agentId}`);
        continue;
      }
      if (call.agentId && call.agentId !== result.agentId) {
        errors.push('Claude child identity changed');
        continue;
      }
      call.agentId = result.agentId;
      const explicitRole = roleName(result.agentType);
      if (result.agentType && (!explicitRole || (call.role && call.role !== explicitRole)))
        call.reason = 'Claude role mismatch';
      call.role ??= explicitRole;
      if (children.has(result.agentId) && children.get(result.agentId) !== call.id)
        call.reason = 'reused Claude instance';
      children.set(result.agentId, call.id);
      call.completed = result.status === 'completed';
      call.timestamp = record.timestamp ?? call.timestamp;
    }
    ...
    try {
      ...
      const identities = new Set(
        child
          ...
          .map((record) => roleName(record.attributionAgent)),
      );
      if (identities.has(undefined) || identities.size > 1)
        throw new Error('unknown/conflicting child role');
      const attribution = [...identities][0];
      if (attribution && call.role && attribution !== call.role)
        throw new Error('child role differs from invocation');
      ...
      call.messages = child
        ...
        .map((record, index) => ({
          ...
          index,
          tools: (Array.isArray(record.message?.content) ? record.message.content : [])
