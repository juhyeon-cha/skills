...
async function contract(identity) {
  ...
  for (const name of ['settings.json', 'settings.local.json']) {
    ...
    if (
      settings?.hooks?.PostToolUse?.some(
        (hook) =>
          !hook.matcher || hook.matcher === '*' || new RegExp(hook.matcher).test('EnterWorktree'),
      )
    )
      throw new Error(
        'LEGACY_HOOK_UNVERIFIED: move preparation to .harness.json bootstrap and exclude EnterWorktree from the repository hook; hook existence is not readiness',
      );
  }
  ...
  for (const input of config.preparation?.inputs ?? []) {
    ...
    if (!location.startsWith(identity.top + path.sep) || !(await fs.stat(location)).isFile())
      throw new Error('preparation input must be a regular file inside the workspace');
    digest.update('\0' + input + '\0').update(hash(await fs.readFile(location)));
  }
  return {
    command,
    fingerprint: digest.digest('hex'),
    timeout: config.preparation?.timeout_ms ?? 300000,
  };
}
export async function preparationStatus(identity) {
  ...
  const lock = await fs.stat(paths.lock).then(
    () => true,
    (error) => {
  ...
  if (!current.command) {
    if (lock)
      throw new Error(
        'PREPARE_BUSY_OR_UNREACHED: existing preparation lock; no-command config cannot bypass it',
      );
  ...
  const ready =
    ...
    receipt.top === identity.top &&
    receipt.gitDir === identity.gitDir;
  return {
    preparation: ready ? 'ready' : 'not-ready',
    ready,
    canDelegate: ready,
...
async function acquire(paths, token, deadline, identity, env) {
  ...
  for (;;) {
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          token,
          pid: process.pid,
          ...(process.platform === 'win32' ? windowsOwner(env) : {}),
        }),
        { flag: 'wx' },
      );
    ...
    if (
      ...
      const gone = (candidate) =>
        process.platform === 'win32'
          ? windowsOwnerGone(candidate, { cwd: identity.top, env })
          : Promise.resolve(!alive(candidate.pid) && !alive(-candidate.pid));
      if (await gone(owner)) {
        ...
        if (
          ...
          observed?.pid !== owner.pid ||
          observed?.job !== owner.job
        )
          continue;
        if (joined === owner.token)
          throw new Error('PREPARE_JOINED_CRASH: shared preparation worker crashed; retry');
        ...
        try {
          await fs.mkdir(path.join(paths.lock, 'recovery'));
        } catch (error) {
          if (error.code === 'ENOENT') continue;
          if (error.code !== 'EEXIST') throw error;
          if (Date.now() >= deadline)
            throw new Error('PREPARE_LOCK_UNREACHED: abandoned recovery claim');
          await pause(50);
          continue;
        }
        const recovery = path.join(paths.lock, 'recovery', token);
        try {
          await fs.writeFile(recovery, '', { flag: 'wx' });
        } catch (error) {
          if (error.code === 'ENOENT') continue;
          throw error;
        }
        const claims = await fs.readdir(path.dirname(recovery));
        if (claims.length !== 1 || claims[0] !== token)
          throw new Error('PREPARE_LOCK_UNREACHED: competing recovery; inspect lock manually');
        ...
        if (
          ...
          await fs.rmdir(path.dirname(recovery));
          continue;
        }
    ...
    if (Date.now() >= deadline)
      throw new Error(
        'PREPARE_BUSY_OR_UNREACHED: owner/process group still alive or incomplete lock; no lock stolen',
      );
    await pause(50);
  }
}
...
async function groupHasChildren(env) {
  ...
  if (result.error || result.status !== 0)
    throw new Error('PREPARE_UNREACHED: cannot inspect POSIX process group');
...
export async function runPreparation(identity, { env = process.env, say = () => {} } = {}) {
  ...
  try {
    ...
    if (joined && outcome?.token === joined && outcome.fingerprint === current.fingerprint)
      throw new Error('PREPARE_JOINED_FAILURE: shared preparation failed; retry');
    const existing = await read(paths.ready);
    if (
      existing?.version === 1 &&
      existing.fingerprint === current.fingerprint &&
      existing.top === identity.top &&
      existing.gitDir === identity.gitDir
    )
      return { preparation: 'ready', ready: true, canDelegate: true, reused: true };
    await fs.rm(paths.ready, { force: true });
    ...
    timer = setTimeout(() => {
      say('PREPARE_TIMEOUT');
      if (process.platform === 'win32') process.exit(124);
      else process.kill(-process.pid, 'SIGKILL');
    }, current.timeout);
    const result = await runCommand(current.command, { cwd: identity.top, env });
    clearTimeout(timer);
    timer = undefined;
    if (await groupHasChildren(env))
      throw new Error('PREPARE_BUSY: bootstrap descendants still running; lock retained');
    say(result.stdout.toString() + result.stderr.toString());
    if (result.status !== 'exited' || result.code !== 0)
      throw new Error(`부트스트랩 실패: ${result.error?.message || result.signal || result.code}`);
    if ((await contract(identity)).fingerprint !== current.fingerprint)
      throw new Error('PREPARE_INPUT_CHANGED: no ready receipt; retry');
    const receipt = {
      version: 1,
      fingerprint: current.fingerprint,
      top: identity.top,
    ...
    await fs.writeFile(file, JSON.stringify({ token, fingerprint: initial.fingerprint }), {
      flag: 'wx',
    });
...
export async function prepareWorkspaceIdentity(
  identity,
  { env = process.env, say = () => {} } = {},
) {
  ...
  const child =
    process.platform === 'win32'
      ...
      : spawn(process.execPath, [worker, identity.top], {
          cwd: identity.top,
          env,
          detached: true,
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code, signal) => {
      if (code !== 0)
        return reject(
          new Error(
            `PREPARE_FAILED_OR_CRASHED: ${signal || code}; workspace preserved; retry after owner/group exits${diagnostics ? '\n' + diagnostics : ''}`,
          ),
        );
      try {
        ...
        if (
          ...
          typeof result?.canDelegate !== 'boolean' ||
          !['ready', 'not-configured'].includes(result.preparation)
        )
          throw new Error('invalid preparation result shape');
        resolve(result);
      } catch (error) {
        reject(
          new Error(
            `PREPARE_PROTOCOL_UNREACHED: ${output.trim() ? 'invalid worker response' : 'empty worker response'} (${error.message})${diagnostics ? '\n' + diagnostics : ''}`,
          ),
        );
      }
    });
