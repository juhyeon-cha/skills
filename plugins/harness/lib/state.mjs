...
const absolute = (value) => {
  if (!path.isAbsolute(nonempty(value, 'state path')))
...
export function runtimeIdentity(explicit, env = process.env) {
  ...
  if (explicit && env.HARNESS_RUNTIME && explicit !== env.HARNESS_RUNTIME)
    throw new Error('runtime identifiers conflict');
...
export function formatStateContext(metadata) {
  ...
  nonempty(metadata.sessionId, 'state context session');
  return (
    '\n\nHARNESS_STATE_JSON: ' +
    JSON.stringify({
...
export async function resolveState({ runtime, cwd, sessionId } = {}, env = process.env) {
  ...
  return {
    ...
    repo,
    session,
    guardLog: env.HARNESS_GUARD_LOG
      ? absolute(env.HARNESS_GUARD_LOG)
    ...
    legacy: {
      ...
      actors: env.HARNESS_SESSION_ACTOR_LOG || path.join(home, '.claude/harness-session-actor.tsv'),
      cancel: path.join(data, 'stop-resume-cancel'),
      status: 'UNVERIFIED',
    },
  };
}
  ...
  for (;;) {
    try {
      ...
      if (Date.now() >= deadline)
        throw new Error(`state lock unavailable: ${lock}; inspect owner before manual recovery`);
...
export function appendState(file, line, { maxLines = 20000, ...options } = {}) {
  if (typeof line !== 'string' || /[\r\n]/.test(line))
    throw new Error('state record must be one line');
...
export async function bindActor(scope, { ledgerRoot, task, actor }, env = process.env) {
  if (!scope.actors) throw new Error('session id required');
  if (scope.dataSource === 'fallback-unverified')
    throw new Error(
      'claim mapping UNREACHED: pass the active hook data directory as HARNESS_DATA_DIR; claim itself was not changed',
    );
  nonempty(task, 'task');
  nonempty(actor, 'actor');
  const ledgerIdentity = await inspectWorkspace(absolute(ledgerRoot), { env });
  if (ledgerIdentity.common !== scope.common)
    throw new Error('claim mapping UNREACHED: ledger root belongs to another repository');
  ledgerRoot = ledgerIdentity.top;
  const result = await runCommand(
    {
      argv: [
        process.execPath,
        path.join(plugin, 'scripts/ledger.mjs'),
        '--root',
        absolute(ledgerRoot),
        'show',
        task,
    ...
    {
      cwd: scope.top,
      env: { ...env, HARNESS_ROOT: absolute(ledgerRoot), CLAUDE_PLUGIN_ROOT: plugin },
    },
  );
  if (result.status !== 'exited' || result.code !== 0)
    throw new Error(
      'claim mapping UNREACHED: ledger verification failed; claim itself was not changed',
    );
  ...
  if (
    ...
    (row.actor ?? row.assignee) !== actor
  )
    throw new Error(
      'claim mapping UNREACHED: ledger task/status/actor mismatch; claim itself was not changed',
    );
  withStateLock(scope.actors, () => {
    let claims = [];
    if (fs.existsSync(scope.actors)) {
      readActors(scope);
      claims = JSON.parse(fs.readFileSync(scope.actors, 'utf8')).claims;
    }
...
export function cancelSession(scope) {
  ...
  if (scope.dataSource === 'fallback-unverified')
    throw new Error('cancel UNREACHED: pass the active hook data directory as HARNESS_DATA_DIR');
  withStateLock(scope.cancel, () =>
    atomicJson(scope.cancel, {
      runtime: scope.runtime,
      repoKey: scope.repoKey,
      sessionId: scope.sessionId,
    }),
...
export function isCancelled(scope) {
  ...
  if (
    value.runtime !== scope.runtime ||
    value.repoKey !== scope.repoKey ||
    value.sessionId !== scope.sessionId
  )
    throw new Error('cancel scope mismatch');
  return true;
}
export function storeWorkflow(scope, kind, id, value) {
  if (!scope.session || !['call', 'outcome', 'result'].includes(kind))
    throw new Error('invalid workflow scope/kind');
  const file = path.join(
    scope.session,
    'workflow',
    hash(nonempty(id, 'call id')) + '.' + kind + '.json',
  );
  withStateLock(file, () => {
    if (fs.existsSync(file)) throw new Error('workflow record already exists');
    atomicJson(file, {
      runtime: scope.runtime,
      repoKey: scope.repoKey,
      sessionId: scope.sessionId,
