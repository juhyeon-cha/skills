...
export function gitEnvironment(env = process.env) {
  return Object.fromEntries(Object.entries(env).filter(([key]) => !key.startsWith('GIT_') || ['GIT_CONFIG_GLOBAL', 'GIT_CONFIG_NOSYSTEM', 'GIT_ALLOW_PROTOCOL', 'GIT_TERMINAL_PROMPT'].includes(key)));
}
async function git(cwd, args, env = process.env) {
  const result = await runCommand({argv: ['git', '-C', cwd, ...args]}, {cwd, env: gitEnvironment(env)});
  if (result.status !== 'exited' || result.code !== 0) throw Object.assign(new Error(`git ${args[0]} failed: ${result.stderr.toString() || result.error?.message || result.signal}`), {completion: result});
  return result.stdout.toString();
}
export function parseWorktrees(text) {
  return text.split('\0\0').filter(Boolean).map(record => Object.fromEntries(record.split('\0').filter(Boolean).map(line => {
...
export async function inspectWorkspace(location, {exact = true, env = process.env} = {}) {
  ...
  const common = await fs.realpath(path.resolve(top, (await git(top, ['rev-parse', '--git-common-dir'], env)).trim()));
  const gitDir = await fs.realpath(path.resolve(top, (await git(top, ['rev-parse', '--git-dir'], env)).trim()));
  const registrations = parseWorktrees(await git(top, ['worktree', 'list', '--porcelain', '-z'], env));
  const canonical = await Promise.all(registrations.map(async row => ({...row, worktree: await fs.realpath(row.worktree).catch(() => path.resolve(row.worktree))})));
  ...
  const branch = (await git(top, ['symbolic-ref', '--quiet', '--short', 'HEAD'], env).catch(() => '')).trim();
  if ((registration.branch ?? '') !== (branch ? `refs/heads/${branch}` : '')) throw new Error('Git branch and registration disagree');
  const main = canonical[0].worktree;
  const mainCommon = await fs.realpath(path.resolve(main, (await git(main, ['rev-parse', '--git-common-dir'], env)).trim()));
...
async function storyContext(cwd, story, env) {
  let identity;
  try { identity = await inspectWorkspace(cwd, {exact: false, env}); await loadConfig(identity.top); }
  catch (error) { throw new Error(`대상 레포를 찾지 못했다 — ${error.message}`); }
  const root = env.HARNESS_ROOT || identity.top;
  const rows = JSON.parse(await ledger(root, identity.top, ['show', story, '--json'], env));
  if (!Array.isArray(rows) || !Array.isArray(rows[0]?.labels) || !rows[0].labels.includes(`repo:${path.basename(identity.main)}`)) throw new Error('story repo: label does not match target repository');
...
export async function createWorkspace(cwd, story, {destination, env = process.env} = {}) {
  ...
  const target = destination ? path.resolve(cwd, destination) : path.join(context.main, '.claude/worktrees', context.name);
  if (await exists(target) || context.registrations.some(row => row.branch === `refs/heads/${context.expectedBranch}`)) throw new Error('workspace path or branch already exists; inspect and enter it');
  await git(context.main, ['fetch', '--prune', '--quiet', 'origin'], env);
  const base = config.default_branch ? `origin/${config.default_branch}` : (await git(context.main, ['symbolic-ref', '--short', 'refs/remotes/origin/HEAD'], env)).trim();
...
function linked(identity) {
  if (!identity.linked || !/^worktree-[A-Za-z0-9._-]+$/.test(identity.branch)) throw new Error('not a registered story workspace (expected worktree-<name> branch)');
}
...
export async function cleanupWorkspace(cwd, story, {force = false, env = process.env, say = () => {}} = {}) {
  ...
  try { await git(context.main, ['fetch', '--prune', '--quiet', 'origin'], env); }
  catch (error) { throw new Error(`fetch --prune 실패 — ${error.message}`); }
  const branchExists = await git(context.main, ['show-ref', '--verify', '--quiet', `refs/heads/${context.expectedBranch}`], env).then(() => true).catch(error => {
    if (error.completion?.code === 1) return false; throw error;
  });
  if (branchExists) {
    const unpushed = await git(context.main, ['log', '--oneline', context.expectedBranch, '--not', '--remotes=origin'], env);
  ...
  if (branchExists) {
    try { await git(context.main, ['branch', '-D', context.expectedBranch], env); result.removed.push('branch'); }
    catch (error) { error.partial = result; throw error; }
  }
  const marker = path.join(context.main, '.claude/worktrees', `.bootstrapped-${context.name}`);
  if (await exists(marker)) { await fs.unlink(marker); result.removed.push('marker'); }
  return result;
}
