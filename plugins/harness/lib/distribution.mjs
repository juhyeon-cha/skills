...
const hookCommand = (id) =>
  `node "\${CLAUDE_PLUGIN_ROOT}/scripts/hook.mjs" ${id} --runtime codex || exit 2`;
...
export function hookTransport(id, runtime) {
  if (runtime === 'claude')
    return {
      type: 'command',
      command: 'node',
      args: [`\${CLAUDE_PLUGIN_ROOT}/scripts/hook.mjs`, id, '--runtime', 'claude'],
    };
  if (runtime === 'codex')
    return { type: 'command', command: hookCommand(id), commandWindows: windowsHookCommand(id) };
...
function skillName(body) {
  ...
  for (const line of frontmatter.split(/\r?\n/)) {
    ...
    if (!field || fields.has(field[1]))
      throw new Error('unsupported/duplicate skill frontmatter key');
...
export function hookWiring(root = pluginRoot, configFile = path.join(root, 'hooks/hooks.json')) {
  ...
  const rows = [];
  for (const [event, groups] of Object.entries(readJson(configFile).hooks))
    for (const group of groups)
      for (const hook of group.hooks) {
        const definition = definitions.find(
          (entry) =>
            entry.event === event &&
            ['claude', 'codex'].some((runtime) => {
              const transport = hookTransport(entry.id, runtime);
              return (
                hook.command === transport.command &&
                JSON.stringify(hook.args) === JSON.stringify(transport.args) &&
                hook.commandWindows === transport.commandWindows
              );
            }),
        );
        if (
          hook.type !== 'command' ||
          !definition ||
          !fs.existsSync(path.join(root, 'scripts/hook.mjs')) ||
          (definition.script && !fs.existsSync(path.join(root, definition.script)))
        )
          throw new Error('unresolved hook transport/target');
        rows.push({
          event,
...
export function projections(root = pluginRoot) {
  ...
  for (const entry of readJson(path.join(root, 'lib/hook-definitions.json'))) {
    if (seen.has(entry.id) || !/^[a-z-]+$/.test(entry.id))
      throw new Error('duplicate/invalid hook identifier');
    seen.add(entry.id);
    ...
    (codexHooks[entry.event] ??= []).push({
      ...(entry.matcher ? { matcher: entry.matcher } : {}),
      hooks: [{ ...hookTransport(entry.id, 'codex'), timeout: entry.timeout }],
    });
  }
  ...
  return {
    ...
    'hooks/hooks.json': encoded({
      description:
        'Generated Claude exec transport from lib/hook-definitions.json; shared Node handlers.',
      hooks,
    }),
    'hooks/codex.json': encoded({
      description:
        'Generated Codex transport from lib/hook-definitions.json; replaces default hooks to avoid duplicate registration.',
      hooks: codexHooks,
    }),
  };
}
...
export function inspectDistribution(root = pluginRoot) {
  ...
  const skills = fs
    ...
    .map((directory) => {
      ...
      names.add(name);
      return { name, path: relative, hash: digest(body) };
    });
  ...
  for (const hook of readJson(path.join(root, 'lib/hook-definitions.json')))
    if (hook.script && !files[hook.script]) throw new Error('hook implementation missing');
  return {
    root,
    version: manifest.version,
    hash: digest(encoded(files)),
    files,
    skills,
    roles,
    contextHash,
