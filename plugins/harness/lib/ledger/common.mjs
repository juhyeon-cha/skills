...
export function parse(args, values = {}, flags = {}, permissive = false) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (Object.hasOwn(values, arg)) {
      ...
      result[key] = key === 'labels' && result[key] ? result[key] + ',' + args[i] : args[i];
    } else if (Object.hasOwn(flags, arg)) result[flags[arg]] = true;
    else if (arg.startsWith('-')) {
      if (!permissive) fail(`모르는 인자 '${arg}'`);
  ...
  parse(
    ...
    Object.fromEntries(
      Object.entries(writeValues).filter(
        ([, key]) => !['status', 'actor', 'assignee'].includes(key),
      ),
    ),
    { '--silent': 'silent', '--stdin': 'stdin', '--json': 'json' },
  );
export const updateOptions = (args) =>
  parse(
    args,
    Object.fromEntries(
      Object.entries(writeValues).filter(([, key]) => !['labels', 'priority'].includes(key)),
    ),
...
export function filterRows(rows, options) {
  const need = csv(options.labels),
    statuses = csv(options.status);
  const pattern = options.pattern
    ? new RegExp(
        '^' +
          options.pattern.replace(/[.*+?^${}()|[\]\\]/g, (c) => (c === '*' ? '.*' : '\\' + c)) +
          '$',
      )
    : null;
  const filtered = rows.filter(
    (row) =>
      (options.all || options.status || row.status !== 'closed') &&
      (!statuses.length || statuses.includes(row.status)) &&
      (!options.type || row.issue_type === options.type) &&
      (!options.parent || row.parent === options.parent) &&
      need.every((label) => row.labels.includes(label)) &&
      (!pattern || row.labels.some((label) => pattern.test(label))),
  );
...
export const bodyFile = async (file, ctx) =>
  (file === '-'
    ? ctx.input.toString()
    : await fs.readFile(path.resolve(ctx.cwd, file), 'utf8')
  ).replace(/\n+$/, '');
...
export async function fileArguments(argv, ctx) {
  ...
  if (Object.hasOwn(files, '--acceptance-file')) {
    if (command === 'init' || inline.has('--acceptance'))
      fail('--acceptance-file: 지원하지 않는 명령 또는 --acceptance 중복');
  ...
  if (Object.hasOwn(files, '--title-file')) {
    if (
      command === 'update' ||
      inline.has('--title') ||
      (command === 'create' && positional.length)
    )
      fail('--title-file: 지원하지 않는 명령 또는 제목 중복');
    const title = await bodyFile(files['--title-file'], ctx);
    if (!title || title.startsWith('-') || title.includes('\0'))
      fail('--title-file: 제목이 비었거나 인자로 표현할 수 없다');
    if (command === 'create') output.unshift(title);
    else output.push('--title', title);
  }
  return [command, ...output];
}
export async function description(options, ctx) {
  return options.bodyFile !== undefined
    ? bodyFile(options.bodyFile, ctx)
    : options.stdin
      ? ctx.input.toString().replace(/\n+$/, '')
      : (options.description ?? '');
}
...
export async function dependencyPairs(args, ctx) {
  ...
  if (args[0] === '--file')
    return (await bodyFile(args[1], ctx))
      .split(/\r?\n/)
      .filter((x) => x.trim())
      .map((line) => JSON.parse(line))
      .filter(Boolean)
      .map((item) => [item.from ?? item.issue_id, item.to ?? item.depends_on_id]);
...
export function railsFrom(rows, ctx) {
  ...
  if (blank.length)
    ctx.err(`rails: assignee (Notion Assignee) 가 없는 epic — ${blank.join(' ')}\n`);
  for (const row of rows.filter((row) => row.assignee != null))
    for (const label of row.labels.filter((label) => label.startsWith('rail:'))) {
      const id = label.slice(5);
      if (!groups.has(id)) groups.set(id, new Set());
      groups.get(id).add(row.assignee);
    }
  for (const [id, owners] of groups)
    if (owners.size > 1)
      fail(
        `rails: 한 레일의 epic 들이 서로 다른 assignee 를 가리킨다 — ${id}=${[...owners].sort().join('/')}`,
      );
  return [...groups]
