...
export async function summarizeGuardLog(
  ...
  if (!['count', 'rows'].includes(command) || args.length > (command === 'count' ? 1 : 2))
    return {
      code: 2,
      stdout: '',
      stderr: `모르는 하위 명령 또는 인자: ${args.join(' ')} — count | rows [<회차>]\n`,
    };
  const log = env.HARNESS_GUARD_LOG || (await resolveState({ cwd }, env)).guardLog;
  let stderr = env.HARNESS_GUARD_LOG
    ? 'STATE UNVERIFIED: explicit legacy TSV; counts are historical observations\n'
    : '';
  let text = '';
  try {
  ...
  if (!text) {
    ...
    try {
      if (!fs.readFileSync(hook, 'utf8').includes('await guardLog(event ?? {}, rule, env);')) {
        ...
        return {
          ...
          stderr:
            stderr +
            `로깅 없는 guard.mjs 가 발화 중이다 — 훅은 돌아도 로그를 남기지 않는다. 이 빈 로그는 '훅 미실행' 도 '발화 0' 도 아니다. 훅=${hook} 로그=${log}\n`,
        };
      }
    } catch {
    ...
    return {
      ...
      stderr:
        stderr +
        `발화 로그가 없거나 비었다 — 관측 미도달: 훅 실행과 로그 쓰기를 확인해야 하며 '발화 0' 이 아니다 (${why}: ${hook}): ${log}\n`,
    };
  }
  const rows = text
    .trimEnd()
  ...
  if (session && !selected.length)
    return {
      ...
      stderr:
        stderr +
        `회차 ${session} 의 행이 로그에 없다 — 차단 0건이 아니라 그 회차를 못 찾았다 (회전으로 사라졌거나 회차 이름이 다르다): ${log}\n`,
    };
  ...
  if (!blocked.length)
    return {
      ...
      stderr:
        stderr +
        `차단 0건 — 이 범위의 ${selected.length}줄이 전부 통과·SKIP 이다. 못 센 것이 아니라 차단이 실제로 0 이다: ${log}\n`,
    };
  const counts = { ok: 0, truncated: 0, nocmd: 0 };
  const stdout = blocked
    .map((row) => {
      const command = row[5] ?? '';
  ...
  if (!counts.ok)
    stderr += `차단 ${blocked.length}건이 전부 분류 불가다 — 이 범위로는 오탐률을 낼 수 없다. 오탐 0 이 아니라 못 셌다.\n`;
