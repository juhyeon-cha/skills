    ...
    return [
      {
        ...
        path: value,
        kind:
          ledger && index === executableIndex
            ? 'transport'
            : ledger && index > executableIndex && options.includes(words[index - 1])
              ? 'ledger-root'
              : 'target',
      },
    ];
...
export async function commonCommand(command, { pluginRoot, cwd, dialect, env }) {
  ...
  try {
    ...
    relative = path
      .relative(fs.realpathSync.native(pluginRoot), fs.realpathSync.native(words[1]))
      .split(path.sep)
      .join('/');
  } catch {
    return null;
  ...
  const args = words.slice(2),
    result = { entry: relative, effect: 'read', paths: [], writes: [], remote: false };
  const absolute = (value) =>
    typeof value === 'string' && path.isAbsolute(value) && !/[\0\r\n]/.test(value);
  const rootArgs = () => args.length >= 2 && args[0] === '--root' && absolute(args[1]);
  if (
    ['checks/board-check.mjs', 'checks/rules-check.mjs', 'checks/guardrail-check.mjs'].includes(
      relative,
    )
  ) {
  ...
  if (relative === 'checks/ledger-check.mjs') {
    ...
    result.effect = result.remote ? 'remote' : 'read';
    return result;
  }
  if (relative === 'checks/workspace-check.mjs')
    return args.length === 1 && absolute(args[0]) ? result : null;
  if (relative === 'scripts/board.mjs') {
    if (!rootArgs() || args.length !== 3 || !/^(?:all|backlog|adr|\d{4}-S\d{2})$/.test(args[2]))
      return null;
    result.effect = 'projection';
    result.writes = [path.join(args[1], 'docs')];
    return result;
  }
  if (relative === 'scripts/config.mjs') {
    if (!absolute(args[1])) return null;
    if (args[0] === 'validate' && args.length === 2) return result;
    if (args[0] !== 'run' || args.length !== 3 || !['check', 'bootstrap'].includes(args[2]))
      return null;
    ...
    result.effect = args[2] === 'check' ? 'gate' : 'prepare';
    result.paths = [args[1]];
    return result;
  }
  if (relative === 'scripts/guard-log.mjs') {
    while (args[0]?.startsWith('--')) {
      const option = args.shift(),
        value = args.shift();
      if (
        !['--runtime', '--repo', '--data', '--log'].includes(option) ||
        (option === '--runtime' ? !['claude', 'codex'].includes(value) : !absolute(value))
      )
        return null;
    }
    return args.length <= 2 && (!args.length || ['count', 'rows'].includes(args[0]))
      ? result
      : null;
  }
  if (relative === 'scripts/transcript.mjs') {
    while (args.length) {
      ...
      if (
        !value ||
        value.startsWith('--') ||
        (['--projects', '--scope'].includes(option) && !absolute(value))
      )
        return null;
    }
    return result;
  ...
  if (relative === 'scripts/state.mjs') {
    ...
    if (args[0] === '--data') {
      ...
      stateEnv.HARNESS_DATA_DIR = value;
    }
