...
#      (이슈·페이지가 원격에 있고 루트는 HARNESS_ROOT 또는 위로 거슬러 찾은 .harness.json 으로 정한다).
...
#
# 하네스 루트는 lib/harness-root.sh 가 낸다 — HARNESS_ROOT 또는 워크트리에서 위로 거슬러 처음
# 만나는 .harness.json 의 디렉토리다(그 파일이 판별자다). 워크트리는 자기 체크아웃에 그 파일을
# 가지므로 워크트리 자신이 루트다. 워크트리 배선은 루트 판별에 쓰이지 않는다 — beads 백엔드 안에서만
# 산다. 못 찾으면 exit 2 와 stderr 의 "원장 배선 실패" — 헬퍼의 사유를 그대로 싣는다. PostToolUse 는 도구 실행 뒤라 훅 실패가
...
repo="${main##*/}"                            # 클론 디렉토리 이름 — 로그용

# ① 원장 배선 — 루트는 헬퍼가, 배선은 어댑터가. 헬퍼의 stderr(왜 못 찾았나)를 사유에 싣는다.
if ! ROOT="$(cd "$wt" && bash "$PLUGIN_ROOT/lib/harness-root.sh" 2>&1)"; then
  fail "하네스 루트를 찾지 못했다 (워크트리 $wt): ${ROOT:-lib/harness-root.sh rc≠0}. 대상 레포 루트에 .harness.json 을 두고 커밋하라 — 그 파일이 판별자다"
fi
...
# 부트스트랩 명령의 출처는 **대상 레포 자신의 `.harness.json`** 이다 — 게이트 명령·기본 브랜치·원장 좌표와 같은 자리다.
