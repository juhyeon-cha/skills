...
>
> **Where the code is.** The core (skills · agents · hooks · checks · scripts · lib) and these six documents (the plugin's `docs/`) are the plugin `harness@skills`, source `${CLAUDE_PLUGIN_ROOT}` (`plugins/harness` in the skills repo). **Every change to a harness rule, check, or document is therefore a story on the `skills` repo** — a development session in that clone, `EnterWorktree`, PR. What is left outside the plugin is each target repo's own `.harness.json` and the ledger — neither of them a tree anyone commits a harness change into. Paths written as `hooks/…` · `checks/…` · `lib/…` · `docs/…` · `scripts/…` are plugin-relative unless the sentence says otherwise.

## Verdicts and gates

- **The gate is the target repo's own** — the `check` field of the skills repo's `.harness.json` (`bash scripts/check.sh`: `claude plugin validate --strict` per plugin · `shellcheck` over every `*.sh` under `plugins/` · the agent-doc-audit regression over both plugins' documents · the three-way description match). `jq -r .check .harness.json` is what names it, with no fallback — an absent gate must not read as a passing one.
- **`scripts/check-all.sh` is the plugin's own check set** and is run by hand — every `checks/*.sh`, with the exemptions (`guard-check` too slow · `transcript-check` judges outside the tree · `ledger-check` touches the remote) listed with reasons inside that file. The ledger-reading checks reach the ledger through the adapter `scripts/ledger.sh` and find the harness root through `lib/harness-root.sh`, whose marker is the `.harness.json` found walking up from the cwd — so the answer follows the tree the run stands in. `check-all.sh` runs every check **from the caller's cwd** (it only moves itself to the plugin root), so tree-scoped checks judge the tree you stand in. To point a run at a fixture or a copy root, set `HARNESS_ROOT`. Before the marketplace carries the plugin, export `HARNESS_PLUGIN_ROOT=<skills clone>/plugins/harness`. **What each check sees and the fail-open rules are held by [guardrails.md](guardrails.md)** — enforcement talk lives in that one place.
...
- **Tree judgment uses the payload, not an anchor.** `GUARD_ROOT` is the plugin root (`CLAUDE_PLUGIN_ROOT`, else the script's own parent) and no rule uses it to judge a tree — it appears only in internal-error messages and to locate `lib/harness-root.sh`. Rules that need a target repo walk up from the path to the first `.harness.json` — the same discriminator `lib/harness-root.sh` uses — and relative paths are folded against the payload `cwd`. Role rules compare `agent_type` against `harness:<name>` only.
...
- No absolute path is **baked** into a core file. A target repo is wherever its `.harness.json` is — checks build fixture repos in a temporary directory so they verify without touching real clones. The harness root is what `lib/harness-root.sh` prints, or `HARNESS_ROOT`.
- **The harness root travels in the delegation message.** A subagent's cwd is not something the delegating side can count on, so the value is stated rather than derived. A new role or skill that forgets the slot for that value leaves the subagent unable to call `ledger.sh` at all — `HARNESS_ROOT=<harness root>` is the only thing that points it at this harness.
...
- **Repo-owned context** (committed in the target repo): `.harness.json` — the gate command, the default branch, the bootstrap, and the ledger coordinates. The rail and sprint registries are **not** files the core knows — they are the adapter's `rails`·`sprints` answers, backed by whatever the backend keeps (on `beads`, `rails.json`·`sprints.json` at the repo root). Projections are outside git and belong to neither side.
- **What the plugin cannot carry, a person does.** `permissions.deny` (the 10 secret-file patterns) and `permissions.allow` are settings, not plugin components — a tree that needs them puts them in its own `.claude/settings.json`. The plugin itself is enabled by neither — it is installed at **user scope**, so nothing in a project or a clone registers it. The harness root **is** the target repo, recognized by the `.harness.json` committed at its root. **The harness plants no git hook anywhere.**
