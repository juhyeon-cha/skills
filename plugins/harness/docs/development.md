...
- **The gate is the target repo's own** — the `check` field of the skills repo's `.harness.json` (`bash scripts/check.sh`: `claude plugin validate --strict` per plugin · `shellcheck` over every `*.sh` under `plugins/` · the agent-doc-audit regression over both plugins' documents · the three-way description match). `scripts/repo.sh check skills` is what prints it, with no fallback.
- **`scripts/check-all.sh` is the plugin's own check set** and is run by hand — every `checks/*.sh`, with the exemptions (`guard-check` is the full sweep and putting it in whole doubles this gate · `transcript-check` judges outside the tree · `ledger-check` touches the remote) listed with reasons inside that file. `guard-check`'s exemption is **not** "it is slow": measured, it takes about as long as every other check here put together (159.5s against 163.3s), and a 76.6s check runs here with no exemption at all. What the exemption buys is that the gate does not double. Its sections below the threshold do run here, through `checks/guard-fast-check.sh` — the threshold and which section sits on which side are owned by the 「절 범위」 comment in `checks/guard-check.sh`. The ledger-reading checks reach the ledger through the adapter `scripts/ledger.sh` and find the harness root through `lib/harness-root.sh`, whose marker is the `ledger.json` directly under the clone root — machine-fixed, so the answer does not depend on which tree the run stands in. `check-all.sh` runs every check **from the caller's cwd** (it only moves itself to the plugin root), so tree-scoped checks judge the tree you stand in. To point a run at a fixture or a copy root, set `HARNESS_ROOT`. Before the marketplace carries the plugin, export `HARNESS_PLUGIN_ROOT=<skills clone>/plugins/harness`. **What each check sees and the fail-open rules are held by [guardrails.md](guardrails.md)** — enforcement talk lives in that one place.
...
- **The harness root travels in the delegation message.** The worktree is outside the harness (`~/.harness-workspace/<repo>/.claude/worktrees/<worktree name>/`), so it cannot be derived from the path. A new role or skill that forgets the slot for that value leaves the subagent unable to call `ledger.sh` at all — `HARNESS_ROOT=<harness root>` is the only thing that points it at this harness.
...
- **Machine-local context** (differs per machine, never committed): `ledger.json` · `repos.json` directly under the clone root, both written only by `scripts/repo.sh`. The rail and sprint registries are **not** files the core knows — they are the adapter's `rails`·`sprints` answers, backed by whatever the backend keeps (on `beads`, `rails.json`·`sprints.json` under the root). Each target repo's gate command, default branch, and bootstrap are that repo's own `.harness.json`. Projections are outside git and belong to neither side.
- **What the plugin cannot carry, a person and `repo.sh` do.** `permissions.deny` (the 10 secret-file patterns) and `permissions.allow` are settings, not plugin components — a tree that needs them puts them in its own `.claude/settings.json`. The plugin itself is enabled by neither — it is installed at **user scope**, so nothing in a project or a clone registers it. The harness root **is** the clone root — `~/.harness-workspace`, a plain directory recognized by the `ledger.json` directly under it, which `scripts/repo.sh root` writes. **The harness plants no git hook anywhere**, its own root included — that root is not a git repo.
