...
| A15 | "Check that a new constraint does not close an exit" (`roles/reviewer.md`) | finding a conflict between rules is natural-language reasoning. What remains is **persuasion only** |
| A17 | "Which acceptance item does each hunk of the diff belong to" — incidental/excess/intrusion/omission and signal priority (`roles/evaluator.md`) | attribution is natural-language reasoning and **the role definition itself admits false positives**. What remains is **persuasion only** |
