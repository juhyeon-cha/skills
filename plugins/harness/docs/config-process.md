...
`ledger.backend` must explicitly name `github`, `beads` or `notion`. Coordinates remain owned and consumed by the existing ledger adapters. Optional `owner` and `database_id` fields must be nonempty strings when present. Under explicit schema 1, `project` must be a positive safe integer. With no schema version, a canonical positive decimal string such as `"5"` is also accepted and kept as a string, matching the GitHub adapter's project comparison. Leading zeros, whitespace, signs, fractions and values outside the safe integer range are rejected. Missing coordinates can be supplied by adapter initialization and are not fabricated here. `default_branch`, when present, is a nonempty string. A missing `check` cannot be executed.
...
Executable discovery is centralized in `lib/process.mjs`. Explicit relative executable paths resolve against the supplied cwd. POSIX PATH entries retain order, including empty entries meaning cwd; an absent PATH has no implicit fallback. Windows candidates use case-insensitive PATH/PATHEXT names, semicolon separation, drive and UNC paths. No implicit Windows current-directory search is added. Drive-relative and current-drive-rooted executable paths are rejected as ambiguous. `.cmd`/`.bat` files use the explicit `cmd.exe /d /s /v:off /c` adapter, with no implicit Node shell. Its quoted argument subset rejects control characters, double quotes, expansion/operator characters (`% ! ^ & | < > ( )`) and trailing backslashes before launch. Use a native executable such as Node with a script argument for arbitrary argv. Other Windows script extensions are refused. Windows candidate fixtures on another OS establish lexical behavior only; native Windows execution support requires a Windows run.

Workspace `enter`/`prepare` consumes this command contract and owns readiness, fingerprints and process supervision; see [workspace.md](workspace.md). Optional `preparation` accepts only `timeout_ms` (1 through 2147483647) and `inputs` (an array of relative file paths without parent traversal). Actual input resolution rejects files outside the workspace. The direct config CLI does not create readiness receipts; use workspace preparation for that. Existing ledger adapters retain their backend readers.
