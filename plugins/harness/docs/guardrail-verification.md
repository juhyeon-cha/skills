...
When confirmed session work remains `in_progress`, Stop emits an advisory NOTICE. It may block only with session-scoped continuation enabled after an explicit user request. Its paths include `NOTICE`, `BLOCK`, `IDLE`, `RECURSE`, `GAVE_UP`, `ORACLE_FAIL`, `CANCEL`, `VERIFY_PENDING`, `SCOPE_RECOVERED` and `SCOPE_FAIL`. SCOPE_RECOVERED continues to the owned-task judgment and writes two records. SCOPE_FAIL records one observation, prints an ownership-unverified diagnostic and allows termination without claiming completion; unavailable state produces an explicit UNREACHED diagnostic instead.
...
**Only ledger-confirmed actor bindings establish scope.** After claim succeeds, `state.mjs bind` re-reads the task and verifies its status and actor; the state contract suite tests this boundary. PreToolUse writes no successful mapping. S7 supplies isolated matching, mismatched and recoverable scope records and removes narrowing in a source mutation. Recovery follows [state.md](state.md); missing or legacy-only evidence cannot establish ownership and ends with SCOPE_FAIL.

**Verification-pending and just-delegated share one mark contract** [lineage `harness-2a5.4.2` · `harness-o59`]. `VERIFY_PENDING: <commit>` marks implementation awaiting verification; `DELEGATED: <milestone>` marks a batch just delegated. The latest matching marker line wins, so later prose does not erase a mark and a later DELEGATED supersedes VERIFY_PENDING. All marked issues pass (mixed marks included); an unmarked issue produces NOTICE by default, or BLOCK with explicit continuation enabled, with counts in the reason. S7 tests each marker, prose, mixed/rework/missing marks and independent marker-removal mutations. This supersedes the historical last-nonempty-line behavior. A malformed list is ORACLE_FAIL, never zero work.
...
1. **The oracle is ledger-wide; blocking requires owned scope.** A missing mapping triggers recovery before Stop decides. If ownership remains unverified, SCOPE_FAIL allows termination with a diagnostic. A session whose `cwd` reaches no ledger passes with ORACLE_FAIL. Neither outcome proves that work is complete.
...
6. **Binding is separate from claim execution.** Failure to persist a verified binding leaves the successful ledger claim intact and the state mapping UNREACHED. Stop attempts scoped recovery and otherwise allows termination with a diagnostic; a failed claim is never inferred as success from PreToolUse.
7. **An unbound claim may escape Stop enforcement.** Without a verified binding or usable recovery copy, Stop allows termination with SCOPE_FAIL. The actual ledger actor must be supplied to bind; PreToolUse command text and implicit actor defaults cannot establish ownership. Missing evidence does not prove that this session has no work.
8. **Recovery copies are ordinary local state.** If the mapping disappears, Stop rechecks the scoped recovery copy against current ledger ownership before restoring it. If both copies are lost or unusable, explicit bind is required. A subsequent claim alone does not restore the mapping. Actor bindings and recovery copies are not rotated with diagnostic logs and are not tamper-proof against the same OS user.
