...
**Every state file is under `${HARNESS_DATA_DIR:-~/.claude/plugins/data/harness}`** — `stop-resume.log` · the cancel entrance `stop-resume-cancel` (claimed as `stop-resume-cancel.<session_id>` by the first session that sees it) · `ralph-cancel`. Nothing is written into a project tree or a worktree, so a worktree being removed loses no record, and a session opened in any repo or worktree writes to the same place. If the directory cannot be created the guard passes without judging and says so on stderr.
