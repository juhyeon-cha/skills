...
- **The registry's own integrity is not visible by running the hook — assert it from the source.** A mistyped key like `RULES+=("Bash:r_pusk")` printed `command not found` to stderr with **exit code 0**, and `PreToolUse` reads rc=0 as pass, so one rule went dark without a signal. The dispatcher now asserts `declare -F "$fn"` **before** matcher comparison and denies if absent. The reverse direction (**function exists → registered**) is `tests/harness/guard-check.sh`'s `registry_intact`, which derives the `r_`-prefixed function set from the source and asserts four things: every defined rule function is registered · every `RULES+=` comes after `RULES=()` · every registered name has the `r_` prefix · the derived set is non-empty. New rules default to "checked" (inverted polarity). Measured [`harness-uhy.3.5`]: a copy defining a rule function without `RULES+=` is rc=0 under the old gate and rc=1 now.
- **The guard fails closed on environmental unavailability.** Missing Node or `jq` and unreadable input produce rc 2 / `UNREACHED`. `guardrail-check.sh` has a separate legacy check-level skip when `jq` is absent; that skip does not establish guard enforcement. Partial or skipped checks are not evidence of runtime support.
...
**The ledger is reached through `lib/harness-root.sh`** — `HARNESS_ROOT`, else the first `.harness.json` walking up from the cwd. Not found → `ORACLE_FAIL` and pass (never a fallback to 0 — "could not read" must not read as "nothing in progress"). **`ORACLE_FAIL` is a pass path**: outside a repo that carries that file this guard is effectively off, and "the guard is on" must not be read there.

**Only ledger-confirmed actor bindings narrow the oracle.** After claim succeeds, `state.mjs bind` re-reads the task and verifies its status and actor. PreToolUse writes no successful mapping. Missing or legacy-only bindings leave the whole-ledger scope and emit SCOPE_FAIL. S7 uses the actual bind command with an isolated adapter, and removes the narrowing line in its negative control.
...
2. **Nobody checks that it fires.** S2 is a wiring check (`hooks.json` ↔ files), not a firing check. S7 runs the hook **directly** with Stop payloads and a fake oracle — explicit paths with expected record counts, negative control (two runs differing only in the oracle), A/B against the wiring — but **what it cannot see is the runtime**: whether the runtime calls this hook at stop, and whether `decision: "block"` actually prevents the stop. S7 runs in a sandbox `cwd` with `GIT_DIR` and `GIT_INDEX_FILE` unset and `HARNESS_DATA_DIR` redirected, so it never touches the real log.
...
1. **The loaded `guard.sh` has the logging call.** Source changes do not establish execution in an already-open installed session. Use the runtime diagnosis contract to distinguish expected source from loaded artifact; legacy TSV counts have unverified scope.
...
| no log, and **the inspected `guard.sh` has the logging call** | 1 | Observation UNREACHED: check hook execution, selected path and logging failure; absence alone does not prove the hook never ran |
