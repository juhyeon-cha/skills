...
| C. shared ledger | UC-7 | a planning change to the registries goes out on its own channel |
...
3. `bash "$(<plugin>)/scripts/board.sh" all` redraws the local projections (outside git).
4. If a registry changed, commit **only that**. Otherwise no commit.
5. **With no unresolved decision in this cycle, without instruction**: `git push` — the harness-root `pre-push` runs `ledger-check` in write mode, which reflects the ledger through `ledger.sh sync-check --push` (on `beads` that is `bd dolt push`; on `github`·`notion` there is nothing to send). With an unresolved decision and no user approval, stop after 4.
6. **Under the same condition** open the PR — the endpoint of the planning channel is a PR too. **A person merges.** The three cases are the two exceptions of `CLAUDE.md` "절대 금지"; steps and failure handling are `harness:develop` "사이클 종결".

**Pass criteria**

- `bash "$(<plugin>)/checks/board-check.sh"` → rc 0
- if there is a commit, it carries registries only: `git show --name-only --format= HEAD | grep -cvE '^(sprints|rails)\.json$'` prints 0
- after the push, the tip of `git ls-remote origin refs/heads/<branch>` == `git rev-parse HEAD`
...
1. Clone the harness repo. `ledger.json` comes with it and its `backend` decides what steps 2–3 mean; on `beads`, `.beads/embeddeddolt/` (the ledger data) is gitignored and does not come along.
2. **`beads`**: `ledger.sh bootstrap --dry-run`, then `ledger.sh bootstrap`. **`github`**: nothing to restore — `gh auth status` rc 0 with the `project` scope. **`notion`**: nothing to restore — export `NOTION_TOKEN` on this machine (the token never travels in the clone).
...
- the rows for the new rule in [guardrails.md](guardrails.md) section 1 (what it blocks · what it cannot) land in the harness repo in the same story, and the limits that stay rc=0 get a pinned fixture in `guard-check.sh`
