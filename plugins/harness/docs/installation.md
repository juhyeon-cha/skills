...
Native registration is optional for the generic execution path in [roles.md](roles.md). When selecting native execution, register native agents using `node <installed root>/scripts/roles.mjs register codex <absolute CODEX_HOME/agents directory>` and save the JSON receipt. Plugin agent auto-discovery is not assumed. Keep one registration scope; verify other project/user agent directories and effective runtime discovery for duplicate names. The role contract's restricted TOML verifier rejects unsupported files in a shared directory. On update, compare old files against the old receipt first: preserve any differing/foreign file and resolve the conflict; remove only verified owned old projections, regenerate from the new installed root, and replace the receipt. The generator refuses a differing file rather than overwriting it. Roll back the installed artifact and its matching generated roles/receipt together.
