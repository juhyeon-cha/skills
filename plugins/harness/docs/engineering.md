...
| **Core document** — rules, structure, procedure **an install reads** | the plugin (`skills/` · `roles/` · `hooks/session-context.md` · `docs/`) | the plugin is what every install receives. Only sentences that change the next person's behavior |
