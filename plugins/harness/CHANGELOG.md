...
**폭 판단 — MINOR (사용자 명시 요청).** 역할 실행과 검증 절차를 개선한다. 일반 위임용 `scripts/delegation.mjs`와 관련 실행 기록 계약의 제거는 저장소 정책상 MAJOR 대상이지만, 이번 요청에 따라 MINOR로 발행하며 아래 전환 안내를 제공한다.

### 역할 실행과 검증

- 공통 역할 본문을 `roles/`에, Claude·Codex·Antigravity의 설정을 `native/`에 분리하고 배포 산출물을 일관되게 생성·검사한다.
- 구현·리뷰·평가 역할이 원장 태스크뿐 아니라 일반 요청, 하위 작업 경로, 지정한 미커밋 변경도 처리한다. 원장·원격 승인과 평가자 읽기 전용 경계는 유지한다.
- 일반 하위 에이전트는 런타임 도구로 직접 호출한다. 역할 등록, SIGNAL·호출 기록·재시도 표식은 일반 작업의 필수 조건에서 제외하고 명시적으로 선택한 감사에만 적용한다.
- 기본 Stop에서 원장 조회를 제거하고, 명시적으로 요청한 세션 계속 실행만 유지한다. 절차 진단과 실제 충돌·필수 검사 실패를 구분한다.
- 모델 요청 관찰과 전사 진단을 개선하고 비실행 메타데이터를 수행 근거로 세지 않는다. 불완전한 호출이나 알 수 없는 형식은 미검증으로 남긴다.

### 기존 설치 전환

- 플러그인 업데이트 후 새 세션을 시작한다. 기존 `delegation.mjs` 호출이나 일반 위임 기록 자동화는 호스트의 하위 에이전트 도구와 실제 응답 처리로 전환한다.
- 별도로 생성한 native 역할 파일은 기존 영수증과 대조해 사용자 변경을 보존한 뒤 새 설치본에서 재생성한다. 절차는 `docs/installation.md`와 `docs/runtime-roles.md`를 따른다.
- 저장소 설정과 원장 자료는 유지한다. 현재 세션의 실제 역할 로딩·호출은 설치 후 별도로 확인해야 한다.
...
일반 실행은 플러그인 업데이트로 적용된다. native 역할 파일을 생성해 사용하는 설치는
역할 지침과 모델 상속 변경을 받으려면 기존 파일·영수증을 검증한 뒤 재생성해야 한다.
이 수동 갱신 경계를 포함해 요청한 PATCH로 발행한다.

### 위임과 검증

- 역할을 식별하지 못한 자식도 기본 파일·Git 조회와 내부 협업 보고를 수행할 수 있다.
  역할이 필요한 명령과 쓰기에는 기존 역할 확인을 유지한다.
- 단독 조사·리뷰는 원장 태스크와 관리형 호출 감사 없이 결과와 검토 범위를 보고한다.
- 관리형 위임의 브랜치·HEAD·clean 검사를 시작과 완료 시점에 수행한다.
- 독립 리뷰어를 후속 수정 검토에 재사용하고, 성공한 재시도는 이전 실패 기록을 보존한 채
  해소 관계를 남긴다. 오래된 호출에서 충돌하는 재사용은 실행 전에 거절한다.
- 선호 모델을 사용할 수 없거나 가용성이 불명확하면 런타임 설정을 상속한다.
  사용자가 지정한 모델은 그대로 유지한다.
- 회귀 테스트가 있는 작은 버그 수정은 품질·인수 검증을 통합할 수 있다.
  권한·데이터 손실 등 위험한 변경은 분리 검증하며 작업자의 summary·phase 중복 기록을 줄인다.

## 2.3.4 — 2026-09-12

**폭 판단 — PATCH (사용자 지시).** 기존 실행·검증 경로의 오탐과 과도한 절차를
수정한다. generic 실행은 플러그인 업데이트로 적용된다. native 역할 파일을
생성해 사용하는 설치는 변경된 역할 지침을 받으려면 기존 파일·영수증을 검증한 뒤
재생성해야 한다. 이 수동 갱신 경계를 포함해 요청한 PATCH로 발행한다.

### 실행과 검증 절차

- 일반 Codex collaboration의 prompt-only 역할 실행을 기본으로 허용하고,
  native 실행 불가 시 명시적 native·권한 강제 요구가 없으면 자동 선택한다.
  구현자와 평가자의 독립성 및 호출·커밋·결과 증거 검증은 유지한다.
- 동작을 바꾸지 않는 낮은 위험의 문서 수정에는 독립 평가자 한 명이 품질과
  acceptance를 함께 확인할 수 있다. 실행 지침·권한·계약·데이터 변경은 분리 검증한다.
- 재시도는 누적 기록을 유지하며 진전과 사용자 예산으로 판단한다. batch 개수,
  출력 길이, 무관한 스킬 전수 읽기와 독립 보고서의 PR 의무를 완화한다.
- 필요한 보조 변경의 범위를 명확히 하고, 실행 결과와 상태를 summary의
  --state-file로 한 번에 저장한다. Git commit·claim까지의 원자성은 보장하지 않는다.

### 정상 작업을 막던 경계 수정

- 읽기 검색의 정확한 /dev/null 출력 폐기를 허용하고 파일 쓰기·append·명령 치환
  차단을 유지한다. 파라미터 확장의 정상 대입 오탐도 수정하며 실행 낱말 검사는 유지한다.
- Stop은 세션의 확인된 바인딩 복구 사본을 현재 원장 소유권과 대조한다.
  복구 불가 시 미확인 상태를 기록·알리고 종료를 허용하며 완료로 판정하지 않는다.
- 타 에이전트 TOML의 무관한 값·배열·여러 줄 문자열·table·dotted key를 허용한다.
  유니코드 이스케이프를 포함한 실제 역할 이름 충돌과 소유 파일 무결성은 검사한다.
- 스쿼시 머지 후 정리 시 현재 branch tip과 정확히 일치하는 머지 PR을 확인해
  이미 반영된 브랜치를 미푸시로 오인하지 않는다. 문서 검사에는 .mjs를 포함한다.

### 검증 범위

- 역할·상태·원장 기록·읽기 분류·Stop 복구·TOML 경계 회귀 및 독립 리뷰/평가를 수행했다.
- CI의 플랫폼 코어 검사는 실제 새 세션의 플러그인 로드·훅 발화를 보증하지 않는다.
  업데이트 뒤 새 세션에서 설치·정적·로드·실행 증거를 구분해 확인한다.
...
설치자가 변경할 설정이나 새 명령은 없으며 플러그인 업데이트로 적용한다.

### 등록된 generic 자식에 역할 정책을 연결한다

- Codex 훅은 실제 세션·저장소·데이터 경로에 속한 유효한 delegation 기록을 확인해
  generic 자식의 역할 정책을 선택한다. spawn 전 기록한 dispatch로 첫 도구 호출을
  처리하며, 결과 승인은 기존 bind·complete 검증을 계속 거친다.
- 다른 세션·저장소, 변조되거나 종료된 기록, 중복 매칭은 거부한다. native 역할을
  꾸며내지 않으며 reviewer·evaluator 쓰기 제한과 implementer 원격 작업 제한을 유지한다.
- 등록 전후와 거부 경로를 훅 프로세스까지 회귀 검사하고, 기존 구현으로 되돌리면
  첫 읽기 회귀 검사가 실패함을 확인했다. 실제 Codex 자식 실행은 훅 로딩에 도달하지
  못해 UNREACHED이며, 이 검사는 provider 역할 강제나 독립 리뷰 통과를 입증하지 않는다.

## 2.3.1 — 2026-09-12

**폭 판단 — PATCH (사용자 지시).** 새 원장 하위 명령이 포함되어 기본 규칙상 MINOR에
해당하지만 요청에 따라 PATCH로 발행한다. 기존 설치 설정은 유지되며, 기존 GitHub
Project의 새 뷰 활성화와 필드 동기화는 선택적 적용 절차다.

### 원장 기록과 이름을 정리한다

- 스토리·마일스톤·태스크·결정 제목에 고정 접두사를 붙인다. 백로그는 스프린트에
  배정하지 않은 스토리로 관리하고, 하위 실행 항목은 착수하거나 명시적으로 분해할 때 만든다.
- `ledger state`와 `ledger summary`가 실행 상태와 최신 구현·검증 요약을 본문에 갱신한다.
  일반 진행 댓글을 줄이고, `note`는 차단·결정·판정 변경 이벤트에 사용한다.
  기존 본문과 acceptance를 보존하며 완료 이유는 완료 시 기록한다.
- 기존 기계 상태 기록을 호환하고 GitHub·Notion 기록을 페이지 끝까지 읽는다.
  Beads·Notion의 재할당 시 actor 갱신과 실행 상태 소비도 함께 정리한다.

### GitHub Project 기본 뷰를 제공한다

- 현재 스프린트·백로그·실행·판단 필요의 네 뷰와 전용 Harness Status 필드를 제공한다.
  기존 사용자 뷰를 보존하며 차이를 보고한다.
- `project-setup`과 `project-sync`는 기본적으로 계획만 조회하고 `--apply`로 적용한다.
  기존 Project는 두 명령을 순서대로 적용해 활성화하며, 이후 이슈 상태·라벨을
  Project 필드에 단방향 반영한다. 상세 절차는 `docs/ledger-projects.md`에 있다.
- 대량 동기화의 페이지 처리·부분 실패 재시도·적용 후 수렴 확인을 지원한다.
  기본 탭 순서와 계층 펼침은 GitHub UI에서 설정해야 한다.

### 권한과 회귀 검증을 맞춘다

- 구현자의 본문 상태·요약 갱신만 허용하도록 guard와 계약 검사를 맞춘다.
  reviewer·evaluator의 원장 쓰기 제한은 유지한다.
- 병합 전 Linux·macOS·Windows CI를 모두 통과했다. 원격 본문 갱신은
  전후 충돌 검사를 제공하지만 원격 서버의 원자적 CAS를 보장하지 않는다.
...
스킬도 검사도 하위 명령도 늘거나 줄지 않았고, 설치본이 손으로 할 일이 없다 —
`claude plugin update` 하나로 끝난다.

스토리 skills#217 회고가 승격한 것들이다.

### 가드 거부 메시지가 삭제된 스크립트를 가리키지 않는다

- 클론 루트 차단 메시지에서 "이 층은 `scripts/repo.sh` 가 소유한다" 를 **지웠다.** 그 스크립트는
  2.1.1 에서 걷혔고 배포물에 없다 — **차단당한 에이전트가 읽는 문장**이 없는 파일로 보내고 있었다.
  대체 문장을 두지 않는다: 그 층에는 이제 소유자가 없고, 사람이 레포를 클론하면 생긴다.
- `hooks/guard.sh` 주석과 `docs/guardrails.md` 의 같은 서술도 함께 고쳤다.

### 첫 줄이 SIGNAL 이 아닌 역할 응답의 처분이 생겼다 (skills#53 · skills#86)

- 종전에는 **"목록에 없는 SIGNAL 값"** 만 규율에 있었다. 첫 줄이 아예 `SIGNAL:` 이 아닐 때의
  처분이 없어 오케스트레이터가 본문 아래에서 값을 찾아 진행했다. 이제 그 응답은 판정이 아니고,
  사람 대기로 간다.
- **후속 턴의 경우도 함께 담았다.** `transcript-check` 의 A9 는 역할의 **마지막** 텍스트 블록만
  읽으므로, 앞 턴에 제대로 신호를 낸 역할이 후속 질문에 산문으로 답하면 위반으로 잡힌다.
  그때는 앞선 응답에서 판정을 읽고 후속 턴을 새 판정으로 읽지 않는다.

### 수 규율이 커밋 경로에도 묶인다 (skills#243)

- 커밋 메시지와 코드 주석에 쓰는 수는 **쓰는 시점에 잰다.** 규율의 단일 소유는
  `harness:plan-story` 5절 그대로이고, 그 스킬을 로드하지 않는 구현·리뷰 경로에도 묶인다는
  사실만 `develop` 이 말한다.
- 반복되는 두 형태를 이름 붙였다 — 변경 **전**에 잰 수를 **후** 문장에 옮기는 것, 총계가
  하위 수치의 합과 어긋나는 것. 뒤엣것은 읽는 사람 눈에는 보이고 게이트에는 안 보인다.
