...
try {
  ...
  while (args[0]?.startsWith('--')) {
    const key = args.shift(),
      value = args.shift();
    if (!value || !['--runtime', '--data', '--repo', '--log'].includes(key))
      throw new Error(
        'usage: [--runtime claude|codex] [--data <absolute>] [--repo <absolute>] [--log <legacy TSV>] count|rows [session]',
      );
    if (key !== '--runtime' && !path.isAbsolute(value)) throw new Error(`${key} must be absolute`);
    if (key === '--runtime') {
      if (!['claude', 'codex'].includes(value)) throw new Error('unknown runtime');
      env.HARNESS_RUNTIME = value;
