...
try {
  const input = process.argv.slice(2), env = {...process.env};
  if (input[0] === '--data') {
    input.shift(); const directory = input.shift();
  ...
  if (action === 'guard') {
    ...
    else if (action === 'bind') console.log(JSON.stringify(await bindActor(scope, {ledgerRoot: args[3], task: args[4], actor: args[5]}, env)));
