...
try {
  ...
  if (
    !root ||
    extra.length ||
    !['validate', 'run'].includes(action) ||
    (action === 'validate' ? field !== undefined : !field)
  )
    throw new Error(
      'usage: node scripts/config.mjs validate <repo> | run <repo> <check|bootstrap>',
    );
  ...
  if (action === 'validate') process.stdout.write(JSON.stringify(config) + '\n');
    ...
    if (command === null)
      process.stderr.write('config: bootstrap is not configured; no process started\n');
    else {
      ...
      if (result.status === 'exited') process.exitCode = result.code;
        ...
        process.exitCode =
          result.status === 'signaled' ? 128 + (os.constants.signals[result.signal] ?? 1) : 1;
      }
    }
  }
} catch (error) {
