...
try {
  ...
  const definition = readJson(path.join(pluginRoot, 'lib/hook-definitions.json')).find(
    (entry) => entry.id === id,
  );
  ...
  if (id === 'context') run.stdout = JSON.stringify(sessionContext(pluginRoot));
    ...
    try {
      ...
      run.code = 2;
      run.stderr += `원장 배선 실패 — ${error.message}\n`;
    }
  } else if (!['role-start', 'role-stop'].includes(id))
    throw new Error('registered hook has no native handler');
  const code = run.code;
  let stateContext;
  try {
    ...
    if (id === 'context' && code === 0) {
      ...
      stateContext = {
        ...
        repository: scope.top,
        sessionId: scope.sessionId,
        data: scope.data,
      };
      if (typeof output.hookSpecificOutput?.additionalContext !== 'string')
        throw new Error('session context output missing');
