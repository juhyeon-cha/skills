...
#
# [천장] 자르는 쪽(guard.sh 의 `${ev:0:120}`)이 **문자** 단위인 것은 UTF-8 로케일 전제다.
# LC_ALL=C 로 도는 훅에서는 같은 표현이 바이트로 잘리고, 그렇게 잘린 한글 행은 문자 수가
# 120 미만이라 여기서 `ok` 로 나온다 — 분류 불가가 **아닌** 쪽으로 틀리는 방향이라 위
# 천장보다 나쁘다(사람이 잘린 명령을 온전한 것으로 읽고 판정한다). 미실측 가설이다.
#
