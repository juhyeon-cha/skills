...
try {
  ...
  if (event.tool_name === 'Bash') {
    const workspace = workspaceShellCommand(
      event.tool_input.command,
      fileURLToPath(new URL('./workspace.mjs', import.meta.url)),
      { dialect: event.harness_shell_dialect },
    );
    if (workspace) {
      if (
        ['harness:reviewer', 'harness:evaluator'].includes(event.agent_type) &&
        !['inspect', 'ready'].includes(workspace.action)
      )
  ...
  const paths = event.harness_operations.flatMap((op) =>
    op.kind === 'move' ? [op.source, op.destination] : [op.path],
  );
  if (process.platform !== 'win32' && paths.some((p) => /^[A-Za-z]:[\\/]|^\\\\/.test(p)))
