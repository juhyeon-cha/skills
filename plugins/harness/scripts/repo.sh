...
# 워크트리 디렉토리를 대상 레포의 로컬 제외 목록에 넣는다 (멱등).
...
add_local_exclude() {
  ...
  if grep -qxF "$EXCLUDE_LINE" "$exclude_file" 2>/dev/null; then
    echo "  제외 목록: 이미 등재됨 ($EXCLUDE_LINE)"
  else
    printf '%s\n' "$EXCLUDE_LINE" >> "$exclude_file"
    echo "  제외 목록: $EXCLUDE_LINE → .git/info/exclude"
  fi
}
...
case "${1:-help}" in
  ...
  help|-h|--help) sed -n '4,9p' "$0" | sed 's/^# \{0,1\}//' ;;
  *) die "알 수 없는 명령: ${1:-} (add | restore | list | remove | help)" ;;
esac
