...
# 플러그인 루트로 cd 한 채 부르면 대상 레포가 아니라 **플러그인이 사는 트리**가 루트로 잡힌다.
# checks/*.sh 는 전부 자기 위치(BASH_SOURCE)에서 플러그인 루트를 파생하므로 호출자 CWD 에
# 기대지 않는다 — 플러그인 루트로 cd 하는 것은 이 스크립트 자신의 find·bash -n·jq 뿐이다.
#
