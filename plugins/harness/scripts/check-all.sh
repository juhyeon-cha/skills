...
SKIP="
guard-check.sh|전수 자리라 여기 통째로 넣으면 이 게이트가 두 배가 된다 — 실측(skills#223, 깨끗한 직렬 1회) 이 검사 159.5초 대 여기서 도는 나머지 합계 163.3초. 임계 아래 절은 checks/guard-fast-check.sh 가 여기서 돌리므로 그 절들은 매 커밋 보인다. 전수는 hooks/guard.sh 를 고치는 커밋에서 손으로 돌린다 (bash checks/guard-check.sh). 임계와 절 배정은 checks/guard-check.sh 의 「절 범위」 주석이 단일 소유한다.
