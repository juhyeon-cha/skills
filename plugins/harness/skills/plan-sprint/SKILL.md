...
Sources: user instruction, `ledger.sh ready`, the backlog (`ledger.sh list`), external issues. A candidate joins the sprint once **the problem it solves fits in one sentence** — until that sentence exists it stays a candidate.
...
1. Every task in `ledger.sh list -l sprint:<ID> --all` has acceptance.
