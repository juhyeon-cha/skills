...
- **"절대 금지"** — remote reflection only on explicit instruction (**two exceptions**: the ledger reflection tied to `git push`, and the working-branch push and PR creation of a cycle closing with no unresolved decision — from merge onward it is explicit instruction) · no direct edits to the main checkout · **fixing the plugin core also only on explicit instruction** · no judging completion by impression. **Do not mark the items with per-item gate notes** — the full list of gates, their limits, and how to verify them is held by `${CLAUDE_PLUGIN_ROOT}/docs/guardrails.md`, and the section points at that file instead of restating it. `rules-check.sh` C6 asserts that pointer is alive
