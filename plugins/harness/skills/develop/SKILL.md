...
Enter the story worktree with the native **`EnterWorktree`** tool. **`name` is not the story ID** — the tool's `name` accepts only letters, digits, dots, underscores and dashes, so a `github` ID (`<repo>#<number>`) is refused as it stands. Run `${CLAUDE_PLUGIN_ROOT}/lib/worktree-name.sh <story ID>` and pass what it prints: that script is the single place this conversion lives, and everything that derives the worktree name from an ID calls it (`skills#105` → `skills-105`; a `beads` ID like `harness-abc` comes back unchanged, which is why the mismatch never showed there). The worktree is `<clone>/.claude/worktrees/<that name>` on branch **`worktree-<that name>`**, cut from `origin/<default branch>` (the `worktree.baseRef` default, `fresh`); the tool does not take a branch name. When `git worktree list` already shows that path (picking up an interrupted cycle), enter it with `path` instead of `name`. Do not run `git worktree add` by hand — the tool and its hook own creation.
...
**Hold the worktree path and the harness root absolute path for the whole cycle.** The worktree sits outside the harness (`~/.harness-workspace/<repo>/.claude/worktrees/<worktree name>/` — the name section 2 derived, not the story ID), so a subagent cannot derive the harness root from its path — state it in every delegation, or `HARNESS_ROOT=<harness root> ledger.sh …` does not work. When the clone is missing, register and clone first with `scripts/repo.sh add <url>` and open the session there.
...
- **Stopping from outside**: the loop itself has no marker — what pushes a session back after its turn ends is the stop guard, and its marker is `touch "${HARNESS_DATA_DIR:-$HOME/.claude/plugins/data/harness}/stop-resume-cancel"`. That path is the **entrance** a human touches, and **the owner is in the file name** — when the guard first finds the entrance it `mv`s it to `stop-resume-cancel.<session_id>` in the same directory, binding it to that session, and from then on it **reads only its own place** and passes until the session ends (the `CANCEL` line of `stop-resume.log` there). **It neither reads nor deletes another's place** — concurrent sessions cannot steal each other's markers, and a dead session's leftover is a harmless empty file rather than something to be consumed. So **touch the entrance again every time you switch it off**: a past session's place cannot switch this session off. The guard keeps its state under that data directory, outside every project tree.
...
| Worktree created | `EnterWorktree`'s success message · the hook's rc | the path exists, the branch is `worktree-<the worktree name>` (`lib/worktree-name.sh <story ID>` prints that name — section 2; it is not the ID itself), and inside it `${CLAUDE_PLUGIN_ROOT}/lib/harness-root.sh` prints the harness root |
...
2. **Remote reflection — the working-branch push, then the ledger's. What the second one is, the backend decides.** Only after stage 1's `board-check`·`ledger-check` passed. Push the working branch first (`git push -u origin worktree-<the worktree name>` — section 2, not the raw story ID; the evidence is the tip of `git ls-remote`). Then the ledger, read off `backend` in the root's `ledger.json` — the adapter owns the shape, this section only says when:
   - **`beads`** — run `HARNESS_ROOT=<harness root> bash ${CLAUDE_PLUGIN_ROOT}/scripts/ledger.sh sync-check --push` explicitly. This is exactly the scope that exception one of the session context block "절대 금지" approves: the moment the target-repo push is the instruction, the ledger goes up inside that approval. With no pre-push hook in the target repo, leaving this command out leaves the ledger as the sole local copy. After reflecting, run `ledger-check` once more in read mode and see `확인됨`.
...
|---|---|
| **Planning** (`plan-sprint` 6 · `plan-story` 7) | registry changes (`sprints.json`·`rails.json`) only. The body of a plan is the ledger, so having nothing to commit is normal, and then this channel's remote reflection is the ledger's alone (`beads`: `ledger.sh sync-check --push` · `github`·`notion`: nothing — the ledger is already remote) — it is not tied to `git push`, so it is outside exception one and subject to explicit instruction |
...
- The repo list has its source in the **clone root's** `repos.json` (`name` · `url`) — a machine-local file, not something the harness shares. **Each repo's gate command, default branch and bootstrap are owned by that repo's own `.harness.json`, at its root** — language and build-tool knowledge lives nowhere outside that file, and `repo.sh check <name>` is what prints the gate command.
- **Registering and cloning a repo happen together in `scripts/repo.sh add <url>`.** The clone location is fixed at `~/.harness-workspace/<name>`, beside the registry itself, and no path is written into it.
- At story pickup, the session opened in that repo's clone makes the worktree with `EnterWorktree`, `name` = what `lib/worktree-name.sh <story ID>` prints (section 2 — the ID itself is refused on `github`): `~/.harness-workspace/<repo name>/.claude/worktrees/<that name>/`. The branch is `worktree-<that name>` (the tool decides it from `name`). The session unit is (story, repo), so in a multi-repo story one session per repo runs the same procedure in its own clone.
