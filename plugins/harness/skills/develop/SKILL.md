...
**The session unit is (story, repo).** A development session opens in the target repo's main checkout — wherever that clone lives — and the clone directory name is this session's repo, which must match one of the story's `repo:` labels. A multi-repo story is worked by **one session per repo**; the sessions coordinate through the ledger (the story bead's notes and the tasks' claims), not through each other. Hold the absolute repository root containing the committed `.harness.json` for the whole cycle and pass it as the adapter's explicit `--root`, including in every delegation. `ledger` uses the common command notation above; its subcommands, arguments and JSON keys preserve the existing adapter contract (`ledger --help`). The backend is `ledger.backend` in that repository's `.harness.json`.

- `ledger show <story ID>` — check that its type is epic, that it carries a `repo:*` label, and that **one of those labels is this session's repo**. Otherwise stop and report what is missing.
...
- `ledger children <story ID>` shows the milestone→task tree. If even one task has no acceptance, do not pick it up; go back to the plan-story procedure.

## 원장 기록

Use `${CLAUDE_PLUGIN_ROOT}/docs/ledger-records.md` when recording execution state, results or events. `ledger show` exposes execution state and projects compatible marker lines into notes; legacy marker reads remain supported.
...
0. **Two checks before pickup.**
   ...
   - **Atomic claim**: take it with `ledger update <task ID> --claim --actor <actor value>`. **In batch mode, claim every task of the list under the same actor before delegating** — drop a rejected task from the list. That leaves as many unmarked `in_progress` tasks as the list holds until the implementer leaves its marks, and `rules-check` S22 counts **the tasks of one actor within one story as a single lane**, so that state is not a violation — do not defer the claim to one per task.
     - Before delegating, record `DELEGATED: <milestone ID>` on the claimed tasks. The implementer's `VERIFY_PENDING` replaces it after committing; this records the execution phase for resumption.
     - **The actor's origin is execution state on the story bead.** Pickup verification (section 1, "ACTOR state") reads the last `ACTOR: <this session's repo> <값>` line of the story's notes — use that value as is when it exists (picking up an interrupted cycle — the same actor passes claim idempotently, so resumption happens by itself), and when it does not, make `sess-` + 6 random characters and leave it with `ledger state <story ID> "ACTOR: <레포> <값>"`. An old-format line (no repo) is `DECISION_NEEDED` — section 1 holds the rule.
     ...
     - **Judge claim state from the claim attempt's rc, or from `status` and `assignee` read together.** A rejection is rc=1 with its message on stderr — in full, `Error claiming <task ID>: issue already claimed by <value>`, and the `already claimed` called below is a substring of it. **Read that rc without a pipe**: `$?` after a pipe belongs to the last command, so a rejection reads as 0.
       - To look at the state again later, call `ledger show <task ID> --json` and read **status and ownership together** from the returned row. Held means `in_progress` + an `assignee` equal to that actor **on the `beads`·`notion` backends**; on `github` the `assignee` is the GitHub login of whoever ran the claim, so there use the explicit `actor` field and the claim's own rc, never compare the login with the session actor.
       - **Do not judge from `assignee` alone.** `ledger update --assignee` puts a value in without a claim — `plan-sprint` section 3 assigns story owners that way, so issues whose `status` is `open` while an `assignee` exists are common in the ledger. Reading those as "held" is the misjudgment this passage blocks.
...
2. `IMPLEMENTATION_COMPLETE` → secure the `VERIFY_PENDING` mark first. The mark is `ledger state <task ID> "VERIFY_PENDING: <커밋 해시>"`, and the stop guard (`${CLAUDE_PLUGIN_ROOT}/hooks/stop-resume.sh`) and `rules-check` S22 use it to separate "finished, awaiting verification" from "left half-done" (the execution phase is authoritative; for existing issues without execution state, the last legacy marker is read. Summary updates leave the phase standing, and re-delegation replaces it). It is a one-line fixed string with no backtick and no `$`, so it falls under the inline allowance of "원장에 본문을 넘기는 형태" (this skill). What follows splits by the unit of verification:
   ...
   - **Outside the condition**: verify the worker's saved summary and phase, fill only a missing phase from the reported commit, and go to **the verify-code procedure** for that task.
3. Verification results: local verification records checks and acceptance and closes through verify-implement without delegation. Combined verification handles evaluator `MATCH` and closing inside verify-implement; continue to step 5 without delegating again. In the separate path, `LGTM` → judge acceptance with **the verify-implement procedure** and close the tasks — in batch mode carry the same list and close the tasks that came back `MATCH` together.
...
1. **A multi-repo story runs one integration verification before it is closed.** The per-task gates ran only inside their own repo — a broken cross-repo contract (an API schema, say) still leaves every task closed as MATCH. In the worktrees of every repo involved, run each repo's `check` once more **at the same final commit state**, and upsert the result (exit code per repo) with `ledger summary <story ID> integration --file <file>`. When a divergence shows, do not close the story; report to the human. A single-repo story has no such step.
2. Once every task is one of closed, blocked, or **deferred** (deferred is the third state the user explicitly closed — "결정 상태" (this skill). When the user says "안 한다", transition it with `ledger update <ID> --status deferred`), upsert the result summary (commit list, items not run) with `ledger summary <story ID> completion --file <file>` and close the milestones and the story with `ledger close <ID> --reason-file <file>`. The close reason is the sole completion comment; include the outcome, verification and limitations without a preceding completion note. **With a `deferred` child, the `beads` backend counts it as an open child and refuses to close** — pass the parent with `ledger close <ID> --force` and the dependency with `ledger dep remove`, and leave the reason for the bypass in the close reason ("결정 상태" (this skill)). After closing, redraw the local projection with `board all` (outside git — not a commit target).
...
4. **Worktree cleanup happens after a merge is confirmed, and only on user instruction.** Judging whether it merged and directing the cleanup is the human's part. Run `workspace cleanup <repo> <story ID>` from outside the workspace being removed. The common command owns the removal order: refresh remote refs, check for uncommitted and unpushed work, refuse unresolved preparation locks, remove the worktree and its Git-owned preparation state, delete the local story branch, and remove any legacy bootstrap marker. Each repository session performs its own cleanup. Read the returned JSON to confirm removal; the legacy `workspace-cleanup.sh` wrapper retains its TSV output.
...
How `/loop` relates to this pipeline. `/loop` is a wake-up that re-enters the session on an interval; the cycle's state is not in the session but in the ledger (the claim · the `VERIFY_PENDING` mark · the `RETRY:` execution counters), so a wake-up that arrives after a turn ended picks the cycle up where the ledger says it is. **A wake-up does not stand in for the completion notification of a background delegation** — that notification arrives on its own while the delegation runs, and the two do not collide. The wake-up is the safety net for the other state: nothing is running and the turn has ended with work still open, so the session is reopened to continue.

Stop is advisory by default. Enable its session-scoped continuation only when the user explicitly requests continued execution, using `${CLAUDE_PLUGIN_ROOT}/docs/state.md`. An open task alone does not authorize continuation. The rules below govern a user-requested loop.
...
- **Stopping from inside**: when a signal leading to human wait appears, break the loop and leave the reason for waiting as a `ledger note`. **The list of those signals is single-owned by the "사람 대기" section below — it is not restated here.** Breaking the loop means ending that turn without rearming — rearming is a per-turn choice, not a default.
...
- **The re-review ceiling is persisted with `ledger state`.** A loop replaces sessions, so the orchestrator's memory disappears every iteration — without the persisted counter, **limit exceeded never arrives.** The format and the ceiling value are single-owned by the "재시도 카운터" of the `verify-code` skill — **no number is written here.**
...
- Completion follows the verification path selected by `verify-code`: local evidence, combined independent verification, or separate review and evaluation. Verification uses the milestone batch condition in section 3.
- Close only when acceptance is met and the selected path's evidence is recorded. Include the commit, checks and limitations in the close reason.
...
- **When choosing to link an independent evaluation retry across sessions**, consult "Cross-session independent evaluator recovery" in `${CLAUDE_PLUGIN_ROOT}/docs/transcripts.md` for the supported scope, failure reference and capacity-wait procedure.
...
**When running a gate is itself a side effect, make the side effect opt-in.** `ledger-check` defaults to read-only; native `--push` and the legacy `LEDGER_CHECK_PUSH` switch explicitly enable reflection. The cycle-close procedure below uses the adapter's `sync-check --push` after the working-branch push. No plugin-owned git hook performs it implicitly.
...
- **Handling is the same for every signal**: leave the reason for waiting with `ledger note` and stop. In a loop, break the loop — the means is `develop` "장기 실행".
- **None of these signals shows in a ledger query.** All of them stay only in `ledger note` bodies, with no status transition.
...
1. **Commit, then the ledger check.** Commit that branch's work — the commit gate is the target repo's. Projections are not commit targets: align the local ones with `board all` only. After the commit the orchestrator runs two checks in the worktree: `board-check` (ledger structure) and `ledger-check` (ledger reflection — **read mode**; do not turn on `LEDGER_CHECK_PUSH`. A pass phrase of `앞서 있음(반영하지 않음 — 쓰기 모드 아님)` means stage 2 has something to raise; `확인됨` means nothing; on `github`·`notion` it is `원격 반영 대상 없음` and stage 2's ledger half is empty from the start). Either one rc≠0 is close incomplete (the failure table).
...
3. **PR creation and state check.** After `gh pr create`, confirm the state with `gh pr view --json url,state` and upsert the URL with `ledger summary <story ID> delivery --file <file>`.
...
|---|---|---|---|
| 1 commit | commit gate rc≠0 | close **incomplete** | `ledger note` the gate output in full. In a loop, break it and wait for the human |
| 1 ledger check | `board-check`·`ledger-check` (read mode) rc≠0 | close **incomplete** | `ledger note` the check output in full on the story bead. Do not go to 2 — raising a ledger that has diverged from the remote (a forked lineage · never reflected) fails the automatic reflection too |
| 1→2 gate | the target repo's rules make push·PR something to ask a human about | close **incomplete** | `ledger note` on the story bead which repo's which file says so. Do not go to 2 |
...
| 2 ledger reflection | the reflection rc≠0, or rc=0 while `ledger-check` is not `확인됨`. **`beads` only** — on `github`·`notion` there is nothing to reflect, so this row cannot occur | close **incomplete**. **The working branch is already out** | `ledger note` the command, rc, and the gist of stderr on the story bead. Do not go to 3 — a PR opened now is a PR without its ledger, and if this machine dies the judgment evidence goes with it. Wait for the human |
