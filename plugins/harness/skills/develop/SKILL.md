...
- **세션 간 독립 평가 재개**는 `${CLAUDE_PLUGIN_ROOT}/docs/transcripts.md`의 "세션 간 독립 evaluator 재개"를 따른다. 현재 세션의 실제 좌표와 검증된 실패 참조를 유지하고, 같은 provider 생성 한도가 재발하면 VERIFY_PENDING과 재개 조건을 남긴다. 부모 대리 판정이나 reviewer의 evaluator 전환으로 완료를 만들지 않는다.
