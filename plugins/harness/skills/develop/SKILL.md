...
The plugin's PostToolUse hook (`${CLAUDE_PLUGIN_ROOT}/hooks/enter-worktree.sh`) wires the ledger (`ledger.sh wire-worktree` — for `beads` a `.beads/redirect` to the harness ledger — plus the clone's `.git/info/exclude`) and, when the target repo has no EnterWorktree hook of its own, runs the `bootstrap` of the worktree's own `.harness.json` once (a sibling marker under `.claude/worktrees/` skips the rerun). **A hook failure does not block the tool** — it exits 2, so its stderr lands in your response (PostToolUse feeds only exit-2 stderr back to Claude): `원장 배선 실패` means the harness root (`.harness.json`) was not found. Before delegating, `${CLAUDE_PLUGIN_ROOT}/lib/harness-root.sh` run inside the worktree must print the harness root. Do not bootstrap (install dependencies) by hand.

**Hold the worktree path and the harness root absolute path for the whole cycle.** The worktree is `<clone>/.claude/worktrees/<worktree name>/` — the name section 2 derived, not the story ID. A subagent's cwd is not something this side can count on, so state the harness root in every delegation; without it `HARNESS_ROOT=<harness root> ledger.sh …` does not work. When the clone is missing, `git clone` it (anywhere) and open the session there — it already carries `.harness.json`.
...
- **A gate's verdict is its exit code.** A role's response is read by **you**, not by a machine, from its first line `SIGNAL:` — there is no enforcement, only `transcript-check` A9 counting afterwards. A value not in the list is a safe exit. **When that first line is not `SIGNAL: <VALUE>`, the response is not a verdict — do not hunt for the value further down the body.** Take it to the human-wait table below. A9 counts exactly this as `NO_SIGNAL`, apart from a first-line value outside the role's vocabulary (`BAD_VALUE`), and it reads the role's **last** text block — so a role that already signalled and was then asked a follow-up lands here too. Read the verdict from the response that carried it, not from the follow-up.
...
| **the first line is not `SIGNAL: <VALUE>`** | any role | whether to re-delegate, or to take the verdict from an earlier response that carried one |
...
- At story pickup, the session opened in that repo's clone makes the worktree with `EnterWorktree`, `name` = what `lib/worktree-name.sh <story ID>` prints (section 2 — the ID itself is refused on `github`): `<clone>/.claude/worktrees/<that name>/`. The branch is `worktree-<that name>` (the tool decides it from `name`). The session unit is (story, repo), so in a multi-repo story one session per repo runs the same procedure in its own clone.
