...
2. **Remote reflection — the working-branch push, then the ledger's. What the second one is, the backend decides.** Only after stage 1's `board-check`·`ledger-check` passed. Push the working branch first (`git push -u origin worktree-<story ID>`; the evidence is the tip of `git ls-remote`). Then the ledger, read off `backend` in the root's `ledger.json` — the adapter owns the shape, this section only says when:
