...
export async function checkGuardrails({
  ...
  const temp = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), 'harness-guardrails-')),
  );
  const outputs = [],
    errors = [];
  let checks = 0;
  const check = async (label, fn) => {
    try {
      await fn();
      checks++;
      if (env.GUARDRAIL_VERBOSE === '1') outputs.push(`✓ ${label}`);
    } catch (error) {
  ...
  const isolated = {
    ...
    HARNESS_DATA_DIR: path.join(temp, 'data'),
    HARNESS_GUARD_LOG: path.join(temp, 'guard.tsv'),
  ...
  const git = (...args) => {
    const run = spawnSync('git', args, { env: isolated, encoding: 'utf8' });
    assert.equal(run.status, 0, run.stderr);
  };
  let copyIndex = 0;
  const mutant = async (relative, change) => {
    const copy = path.join(temp, 'copy-' + ++copyIndex);
    fs.cpSync(pluginRoot, copy, { recursive: true });
    const file = path.join(copy, relative),
      before = fs.readFileSync(file, 'utf8'),
      after = change(before);
    assert.notEqual(after, before, 'source mutation did not hit');
  ...
  try {
    git('init', '-q', main);
    fs.writeFileSync(
      path.join(main, '.harness.json'),
      JSON.stringify({ ledger: { backend: 'beads' } }),
    );
    ...
    const shell = (command, role = '') => ({
      hook_event_name: 'PreToolUse',
      tool_name: 'Bash',
      cwd: workspace,
      session_id: 'policy',
      ...(role ? { agent_id: 'child', agent_type: role } : {}),
      tool_input: { command },
    });
    const write = (file, role = '') => ({
      ...shell('', role),
      tool_name: 'Write',
    ...
    await check('S1 definitions, registrations and probes agree in both directions', () => {
      assert.ok(defined.length >= 7);
      assert.deepEqual(defined.sort(), guard.RULES.map((row) => row.run.name).sort());
      assert.deepEqual(defined.sort(), Object.keys(probes).sort());
      assert.equal(new Set(defined).size, defined.length);
    });
    for (const [rule, input] of Object.entries(probes))
      await check(`S1 ${rule} blocks and its registration-only mutant permits`, async () => {
        ...
        const allowed = await copy.module.evaluateGuard(input, { env: isolated });
        assert.equal(allowed.code, 0, allowed.stderr);
      });
    await check('S2 generated metadata has no drift or duplicate registration', () =>
      distribution.inspectDistribution(pluginRoot),
    );
    for (const runtime of ['claude', 'codex'])
      await check(`S2 ${runtime} wiring covers every handler and removal is detected`, () => {
        const file = path.join(
          pluginRoot,
          runtime === 'claude' ? 'hooks/hooks.json' : 'hooks/codex.json',
        );
        const rows = distribution.hookWiring(pluginRoot, file);
        assert.equal(rows.length, 6);
        for (const [event, script] of [
          ['PreToolUse', 'hooks/guard.sh'],
          ['Stop', 'hooks/stop-resume.sh'],
          ['SessionStart', 'hooks/session-context.sh'],
        ]) {
          const matching = rows.filter((row) => row.event === event && row.script === script);
          assert.equal(matching.length, 1);
          if (event === 'PreToolUse') assert.equal(matching[0].matcher, '');
        }
        const raw = JSON.parse(fs.readFileSync(file, 'utf8'));
        delete raw.hooks.Stop;
        const removed = path.join(temp, runtime + '-without-stop.json');
        fs.writeFileSync(removed, JSON.stringify(raw));
        const changed = distribution.hookWiring(pluginRoot, removed);
        assert.equal(changed.length, rows.length - 1);
        assert.ok(!changed.some((row) => row.event === 'Stop'));
        const definitions = JSON.parse(
          fs.readFileSync(path.join(pluginRoot, 'lib/hook-definitions.json'), 'utf8'),
        );
        const declared = definitions
          .flatMap((entry) => (entry.script ? [entry.script] : []))
          .sort();
        assert.deepEqual(
          [...new Set(rows.flatMap((row) => (row.script ? [row.script] : [])))].sort(),
          declared,
        );
      });
    const scripts = [];
    function walk(directory) {
      for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        else if (/\.(mjs|sh)$/.test(entry.name)) scripts.push(file);
      }
    }
    for (const directory of ['lib', 'scripts', 'checks', 'hooks'])
      walk(path.join(pluginRoot, directory));
    await check('S5 executable source population is nonempty', () => {
      assert.ok(scripts.filter((file) => file.endsWith('.mjs')).length >= 20);
      assert.ok(scripts.filter((file) => file.endsWith('.sh')).length >= 10);
    });
    for (const file of scripts)
      await check(`S5 ${path.relative(pluginRoot, file)}`, () => {
        if (file.endsWith('.mjs')) {
          const run = spawnSync(process.execPath, ['--check', file], { encoding: 'utf8' });
          assert.equal(run.status, 0, run.stderr);
        } else if (process.platform !== 'win32') {
          assert.ok(fs.statSync(file).mode & 0o111, 'POSIX wrapper execution bit missing');
          const run = spawnSync('bash', ['-n', file], { encoding: 'utf8' });
          assert.equal(run.status, 0, run.stderr);
        } else
          assert.ok(
            fs.readFileSync(file, 'utf8').startsWith('#!'),
            'legacy POSIX wrapper missing shebang',
          );
      });
    if (process.platform === 'win32')
      outputs.push(
        'S5 POSIX wrappers: source presence only; Bash syntax/execute bits are POSIX compatibility checks, not native runtime dependencies.',
      );
    const database = path.join(temp, 'upstream/.beads/embeddeddolt');
    fs.mkdirSync(path.join(database, 'db'), { recursive: true });
    fs.mkdirSync(path.join(main, '.beads'));
    fs.writeFileSync(path.join(main, '.beads/redirect'), path.dirname(database));
    let ahead = 0,
      miss = false,
      pushFails = false,
      pushes = 0;
    const transport = async ({ argv }, options) => {
      ...
      let stdout = '',
        code = 0;
      if (executable === 'bd' && args[0] === '-C' && args[2] === 'where') {
        assert.equal(args[1], main);
        stdout = miss ? '' : `database: ${database}\n`;
        code = miss ? 1 : 0;
      } else if (executable === 'bd' && args.join(' ') === 'dolt push') {
        ...
        if (args[0] === 'remote') stdout = 'origin https://example.invalid/db\n';
          ...
          stdout = Array.from({ length: ahead }, (_, i) => `commit-${i}\n`).join('');
    ...
    const runLedger = (implementation = ledger.executeLedger, push = false) =>
      ledgerCheck.checkLedger({
        root: main,
        cwd: workspace,
        env: isolated,
        push,
        ledger: (args) =>
          implementation(args, { root: main, cwd: workspace, env: isolated, process: transport }),
      });
    await check('S6 redirect location comes from bd where and zero ahead is judged', async () => {
      const result = await runLedger();
      assert.equal(result.code, 0);
      assert.match(result.stdout, /원격 반영 확인됨/);
    });
    await check('S6 missing ledger is explicit skipped evidence, never verified', async () => {
      miss = true;
      const result = await runLedger();
      miss = false;
      assert.equal(result.code, 0);
      assert.match(result.stderr, /임베디드 원장 없음/);
      assert.match(result.stdout, /건너뜀/);
      assert.doesNotMatch(result.stdout, /확인됨/);
    });
    await check('S6 read mode does not write or hide ahead state', async () => {
      ahead = 3;
      const before = pushes;
      const result = await runLedger();
      assert.equal(result.code, 0);
      ...
      assert.match(result.stdout, /앞서 있음\(반영하지 않음/);
    });
    await check('S6 explicit push changes state and confirms the state afterward', async () => {
      ahead = 3;
      const result = await runLedger(ledger.executeLedger, true);
      assert.equal(result.code, 0);
      assert.equal(ahead, 0);
      assert.match(result.stdout, /이번에 수행함/);
    });
    await check('S6 failed push cannot pretend the ahead state resolved', async () => {
      ahead = 3;
      pushFails = true;
      const result = await runLedger(ledger.executeLedger, true);
      pushFails = false;
      ...
      assert.equal(ahead, 3);
      assert.match(result.stderr, /해소하지 못했다/);
    });
    await check('S6 location and actual push each have independent live mutations', async () => {
      const location = await mutant('lib/ledger/beads.mjs', (text) =>
        text.replace(
          'const database=/^\\s*database:\\s*(.*)$/m.exec(where.stdout.toString())?.[1]',
          "const database=path.join(ctx.root,'.beads/embeddeddolt')",
        ),
      );
      const alteredLocation = await import(
        pathToFileURL(path.join(location.root, 'lib/ledger.mjs')).href
      );
      ahead = 0;
      assert.match((await runLedger(alteredLocation.executeLedger)).stdout, /건너뜀/);
      const nopush = await mutant('lib/ledger/beads.mjs', (text) =>
        text.replace(
          "await ctx.command('bd',['dolt','push'],{cwd:ctx.root,allowFailure:true})",
          "{stdout:Buffer.alloc(0),stderr:Buffer.alloc(0),code:0}",
        ),
      );
      const alteredPush = await import(
        pathToFileURL(path.join(nopush.root, 'lib/ledger.mjs')).href
      );
      ahead = 3;
      const result = await runLedger(alteredPush.executeLedger, true);
      assert.equal(result.code, 1);
      assert.equal(ahead, 3);
    });
      ...
      const result = await implementation(
        { cwd: workspace, session_id: sessionId, stop_hook_active: active },
        {
          ...
          ledger:
            oracle ??
            (async (args) => {
              assert.deepEqual(args, ['list', '--status', 'in_progress', '--limit', '0', '--json']);
    ...
    await check(
      'S7 recursion, idle, oracle failure variants and ordinary block each log',
      async () => {
        await stopCase('recurse', [], 'RECURSE', { active: true });
        await stopCase('idle', [], 'IDLE');
        await stopCase('oracle-fail', [], 'ORACLE_FAIL', { oracle: async () => ({ code: 1 }) });
        await stopCase('oracle-shape', [], 'ORACLE_FAIL', {
          oracle: async () => ({ code: 0, stdout: '{}' }),
        });
        await stopCase('root-fail', [], 'ORACLE_FAIL', {
          rootFinder: async () => {
            throw new Error('harness-root unavailable');
          },
        });
    ...
    await check('S7 cancellation remains session-owned and the cap is bounded', async () => {
      const scope = await scopeFor('cancel');
      state.cancelSession(scope);
      const legacy = path.join(isolated.HARNESS_DATA_DIR, 'stop-resume-cancel');
      fs.writeFileSync(legacy, 'legacy');
      await stopCase('cancel', [marked('')], 'CANCEL');
      await stopCase('cancel', [marked('')], 'CANCEL');
      await stopCase('other', [marked('')], 'BLOCK');
      assert.ok(fs.existsSync(scope.cancel));
      assert.equal(fs.readFileSync(legacy, 'utf8'), 'legacy');
      assert.ok(stop.MAX_BLOCKS > 0);
      for (let i = 0; i < stop.MAX_BLOCKS; i++) await stopCase('cap', [marked('')], 'BLOCK');
      await stopCase('cap', [marked('')], 'GAVE_UP');
    });
    ...
    await check('S7 markers and scope narrowing have independent mutation controls', async () => {
      for (const [marker, replacement, rows] of [
        ['VERIFY_MARK', '  const vp = 0;', [marked('VERIFY_PENDING: a')]],
    ...
    await check(
      'S7 oracle count changes block reason; no claim is invented by PreToolUse',
      async () => {
        const first = await stopCase('count-one', [marked('')], 'BLOCK'),
          second = await stopCase('count-two', [marked(''), marked('')], 'BLOCK');
        assert.match(first.stdout, /1건/);
        assert.match(second.stdout, /2건/);
        const input = {
          ...shell('bd -C /fixture update t --claim --actor pretend'),
          session_id: 'no-claim',
        };
        await judge(input);
    ...
    await check(
      ...
      async () => {
        ...
        let calls = 0;
        const result = await stop.evaluateStop(
          { cwd: workspace, session_id: 'unwritable' },
          {
            env: { ...isolated, HARNESS_DATA_DIR: path.join(blockedPath, 'data') },
            ledger: async (args) => {
              calls++;
    ...
    await check('S7 default state directory is resolved under the isolated home', async () => {
      ...
      fs.mkdirSync(home);
      const fallback = { ...isolated, HOME: home, USERPROFILE: home };
      delete fallback.HARNESS_DATA_DIR;
      const scope = await state.resolveState(
        { runtime: 'claude', cwd: workspace, sessionId: 'default-state' },
        fallback,
      );
      assert.ok(scope.data.startsWith(home + path.sep));
      const result = await stop.evaluateStop(
        { cwd: workspace, session_id: 'default-state', stop_hook_active: true },
        { env: fallback },
      );
      assert.equal(result.code, 0);
      assert.equal(result.stdout, '');
...
if (isMain(import.meta.url)) {
  try {
    ...
    if (args.length && (args.length !== 2 || args[0] !== '--root' || !path.isAbsolute(args[1])))
      throw new Error('usage: guardrail-check.mjs [--root <repository>]');
