...
# 이 두 키가 원장에 0건인지는 **검사하지 않는다**. 면제 목록을 가진 검사들의 "실재하지 않는
...
# ≥1건만 보므로 과소만 잡고 과폭은 못 잡는다(tests/harness/doc-rules-check.sh 의 R-REM·R-WAIT·R-DUP 이 같은 한계를 적는다).
...
# ── 스프린트 라벨의 하위 상속 + 조상 존재 ───────────────────────────
# 어댑터의 create 는 **만들 때만** 부모의 sprint:·rail:·repo: 를 물려준다. 분해된 백로그 스토리를 나중에 편입하면
