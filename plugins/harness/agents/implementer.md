...
1. **Confirm the current path as the first action.** All work happens inside the assigned worktree (`<clone>/.claude/worktrees/<worktree name>/`) and nowhere else. **Above it is the target repo's main checkout**, so check every time that the path is under `.claude/worktrees/<worktree name>` — one level up is the main checkout. Never touch the main checkout.
...
4. Implement. During development run only the relevant tests, narrowed; **at the end run the repo gate once, in full.** The gate command is the `check` field of that repo's own `.harness.json`, at the repo root (`jq -r .check .harness.json` names it). On failure, fix it (3 attempts at most).
...
- **A ledger.sh call without `HARNESS_ROOT=<harness root>`.** Without the variable, root discovery (`lib/harness-root.sh`) walks up from wherever the cwd happens to be, which on a machine carrying more than one harness can be a **different** ledger — there `note` dies loudly on an id mismatch, but `create` succeeds silently.
