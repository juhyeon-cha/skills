...
4. Implement. During development run only the relevant tests, narrowed; **at the end run the repo gate once, in full.** The gate command is the `check` field of that repo's own `.harness.json`, at the repo root (`repo.sh check <name>` prints it). On failure, fix it (3 attempts at most).
