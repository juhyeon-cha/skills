...
1. **Confirm the current path as the first action.** All work happens inside the assigned worktree and nowhere else. Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/workspace.mjs inspect <assigned worktree>` and confirm that `top` matches the assigned path, `linked` is true, and `branch` matches the delegated story branch. Git registration determines identity; the default `.claude/worktrees/<worktree name>/` layout and external linked paths follow the same check. Never touch the main checkout returned as `main`.
