...
자료를 함께 받은 요청이면 `data-analyst` → `report-writer` 순서다. 확정된 값을 넘겨야
문장 쪽이 숫자를 지어내지 않는다. 자료 없이 대화만 있으면 `report-writer` 하나로 끝난다.

**둘 다 HTML 을 조립하지 않고 폰트 임베드·`finalize.py` 도 돌리지 않는다.** 그건 위 절차가
