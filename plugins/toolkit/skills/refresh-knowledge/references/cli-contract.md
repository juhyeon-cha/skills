...
`--version` used alone returns `toolkit_version` from the installed plugin manifest and `implementation_id`. It needs only Python's standard library, without Git, jsonschema, or a project. Missing or invalid manifests fail instead of supplying an inferred version. `--help` and subcommand help return usage text with exit 0.

`implementation_id` is SHA-256 over sorted relative names and bytes of `.py` and `.json` files and `requirements.txt` under scripts, each part prefixed by its 8-byte big-endian length. Install paths, timestamps and pycache do not affect it. It identifies that source set, not a signature or integrity proof for the entire package. Plugin release version, CLI contract version, DB user_version and artifact versions are separate; this contract performs no migration.
