...
| 팀·작성일·작성자 | **이 세 필드로 고정이다.** 이름을 바꾸지 않고, 하나를 빼고 다른 것을 끼워 넣지 않으며, 넷째를 더하지 않는다. 특히 "공유 대상"·수신처·참고 범위는 여기 쓰지 않는다 — 그 정보가 필요하면 본문이나 꼬리말에 쓴다. 세 값 중 못 채우는 것이 있으면 지어내거나 그 필드를 빼지 말고 **사용자에게 묻는다** |
