...
필요한 답을 찾아 읽는 문서는 업무 질문을 제목으로 삼고 조건을 포함한 답을 먼저 제시합니다.
여러 역할이 관여하는 사례는 서로 필요한 계약·정책·범위 결정을 연결해 설명합니다.
...
API 명세·업무 문서처럼 Git 밖에서 얻은 자료는 [관측 기반 지식 축적](plugins/knowledge/references/notebook.md)을
...
[코드 기반 지식 자동화 목표](docs/initiatives/code-driven-knowledge/README.md)는 문서 AC와 독립 리뷰 검증에서 시작해 근거 기반 갱신, 개발 흐름 자동 연결로 확장하는 단기·중기·장기 방향을 정리한다.
