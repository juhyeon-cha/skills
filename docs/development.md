...
- **The commit gate is the target repo's own** — the `check` field of the skills repo's `.harness.json` (`bash scripts/check.sh`: `claude plugin validate --strict` per plugin · `shellcheck` over every `*.sh` under `plugins/` and `tests/` · the three-way description match).
