...
이 문서는 조사 당시의 제안을 보존한다. 현재 실행 계약은 [계약 문서](../contracts/agent-workflow.md), 후속 개발 순서는 [5단계 로드맵](../roadmap.md)을 따른다. 아래 “다음 단계”와 제안 명령은 조사 시점의 표현이다.

후속 결정: 사용자는 Promptfoo와 LLM 응답 평가 도입을 전체 흐름 구현 뒤 고도화 단계로 미루었다. 아래 기능 조사는 참고로 보존하지만 현재 구현·비교 실험 범위에는 포함하지 않는다. 다음 작업은 원본 근거·변경분 계약과 결정적 CLI이다.
...
| 평가 실행 | Promptfoo: 사용자 정의 검사·모델 판정·기존 출력 평가 | 원본 기반 정답·독립 입력·필수 통과 조건 | 전체 흐름 구현 후 고도화 때 재검토 |
...
3. **변경 전후 사례:** 정상 변경, 같은 변경 재실행, 삭제·이름 변경, 오래된 기준, 중간 실패 후 재개에서 영향 목록·수정안·판정·반영을 확인한다.
4. **구현 이후 고도화:** LLM 응답 평가와 Promptfoo의 필요성을 실제 운영 문제에 따라 다시 판단한다. 현재는 평가 실행기를 도입하거나 자체 대체품을 만들지 않는다.
...
권장 출발점은 **기존 Git + 계약 검사 라이브러리 + 작은 CLI + 제한된 에이전트 판단**이다. Promptfoo는 구현 후 고도화 후보로 미룬다. LlamaIndex·Unstructured·Temporal도 검색·입력·복구 요구가 생길 때 검토한다. 이 조사 자체는 라이브러리 버전과 최종 선택을 확정하지 않는다.
