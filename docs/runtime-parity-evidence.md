...
The desktop's current collaboration provider exposes generic prompt-only delegation. The parent observed this worker's reads and file creation, but that does not establish reviewer read-only enforcement. A documented native custom-role execution path must be connected to desktop orchestration and measured there; a CLI subprocess is only an implementation candidate until that exact desktop path is exercised. The shared guard/tool-event path already operates in the current desktop session, but its final role-level acceptance is pending.
